...
static iot_agent_status_t associateKeyPair(mbedtls_pk_context *pk,
                                           sss_object_t *service_private_key,
...
iot_agent_status_t pubSub(iot_agent_context_t *iot_agent_context, const nxp_iot_ServiceDescriptor *service_descriptor)
{
    iot_agent_status_t agent_status = IOT_AGENT_SUCCESS;
                                          ...
                                          uint32_t cert_id)
{
    ...
    {
        sss_status = sss_key_object_allocate_handle(&service_cert, cert_id, kSSS_KeyPart_Default,
                                                    kSSS_CipherType_Binary, cert_size, kKeyObject_Mode_Persistent);
    ...
    sss_status =
        sss_key_store_set_key(keystore->sss_context, &service_cert, cert_data, cert_size, cert_size * 8, NULL, 0);
    SSS_SUCCESS_OR_EXIT_MSG("sss_key_store_set_key failed with 0x%08x.", sss_status);
exit:
    return agent_status;
}

iot_agent_status_t associateKeyPair(mbedtls_pk_context *pk,
                                    sss_object_t *service_private_key,
                                    iot_agent_keystore_t *keystore,
                                    uint32_t key_id)
{
...
iot_agent_status_t iot_agent_demo_mqtt_connect(MqttAgent_Context_t *pMqttAgentContext)
{
    ...
    xNetworkStatus                       = TLS_FreeRTOS_Connect(
        pMqttAgentContext->pNetworkContext, pMqttAgentContext->pHostName, pMqttAgentContext->port, &xNetworkCredentials,
        mqttexampleTRANSPORT_SEND_RECV_TIMEOUT_MS, mqttexampleTRANSPORT_SEND_RECV_TIMEOUT_MS);
    ...
    mqttStatus = MQTT_Init(pMqttAgentContext->pMqttContext, pMqttAgentContext->pTransportInterface, prvGetTimeMs,
                           pMqttAgentContext->prvUserCallback, pMqttAgentContext->pNetworkBuffer);
    ...
    mqttStatus = MQTT_Connect(pMqttAgentContext->pMqttContext, &connectInfo, NULL, mqttexampleCONNACK_RECV_TIMEOUT_MS,
                              &previousSession);
    ...
    ASSERT_OR_EXIT_MSG(pMqttAgentContext->mqttConnected, "MQTT_Connect failed with status=%s.",
...
iot_agent_status_t iot_agent_demo_mqtt_publish(MqttAgent_Context_t *pMqttAgentContext)
{
    ...
    mqttStatus = MQTT_Publish(pMqttAgentContext->pMqttContext, &pubInfo, packetId);
    ASSERT_OR_EXIT_MSG(mqttStatus == MQTTSuccess, "MQTT_Publish failed with status=%s.",
                       MQTT_Status_strerror(mqttStatus));
exit:
    return agent_status;
}
...
iot_agent_status_t iot_agent_demo_mqtt_wait_packet(MqttAgent_Context_t *pMqttAgentContext)
{
    ...
    // to do mm : mqttStatus = MQTT_ProcessLoop(pMqttAgentContext->pMqttContext);
    ASSERT_OR_EXIT_MSG(MQTTSuccess == mqttStatus, "MQTT_ProcessLoop failed with status=%s.",
                       MQTT_Status_strerror(mqttStatus));
exit:
    return agent_status;
}
...
iot_agent_status_t iot_agent_demo_mqtt_subscribe(MqttAgent_Context_t *pMqttAgentContext)
{
    ...
    mqttStatus = MQTT_Subscribe(pMqttAgentContext->pMqttContext, &xMQTTSubscription,
                                sizeof(xMQTTSubscription) / sizeof(MQTTSubscribeInfo_t), packetId);

    ASSERT_OR_EXIT_MSG(MQTTSuccess == mqttStatus, "MQTT_Subscribe failed with status=%s.",
                       MQTT_Status_strerror(mqttStatus));
exit:
    return agent_status;
}
...
iot_agent_status_t awsPubMqttMessage(const nxp_iot_ServiceDescriptor *service_descriptor)
{
    ...
    MQTTFixedBuffer_t networkBuffer         = {.pBuffer = sharedBuffer, .size = AGENT_NETWORK_BUFFER_SIZE};
    ...
    while (retryCount < 5)
    {
        IOT_AGENT_INFO("Attempt %d for connecting to AWS service '%s'...", retryCount + 1,
    ...
    ASSERT_OR_EXIT_MSG(publishFails < (PUBLISH_ATTEMPTS / 2), "More than or equal to %d publish attempts failed (%d).",
                       (PUBLISH_ATTEMPTS / 2), publishFails);
...
#define WATSON_ECHO_CLIENT_ID \
    "dummy" // For IBM watson ClientID is optional (IBM verifies client from Device certificate)
...
#ifdef NXP_IOT_AGENT_USE_COREJSON
static iot_agent_status_t get_value_from_tag(
    char *js, size_t js_size, const char *key, size_t key_size, char *value, size_t max_value_size)
{
    iot_agent_status_t agent_status = IOT_AGENT_SUCCESS;
    char *value_ptr;
    ...
    ASSERT_OR_EXIT_MSG(JSON_Search(js, js_size, key, key_size, &value_ptr, &value_size) == JSONSuccess,
                       "Error in JSON string parsing");
                                      ...
                                      MQTTDeserializedInfo_t *pxDeserializedInfo)
{
    if ((pxPacketInfo->type & 0xF0U) == MQTT_PACKET_TYPE_PUBLISH)
    {
        ...
        else if (strcmp(status, "assigned") == 0)
        {
            ...
            get_value_from_tag(payload, payloadLength, "registrationState", strlen("registrationState"),
                               registrationState, payloadLength);
            get_value_from_tag(registrationState, strlen(registrationState), "registrationId", strlen("registrationId"),
                               reg_info->registrationId, sizeof(reg_info->registrationId));
            get_value_from_tag(registrationState, strlen(registrationState), "assignedHub", strlen("assignedHub"),
                               reg_info->assignedHub, sizeof(reg_info->assignedHub));
            get_value_from_tag(registrationState, strlen(registrationState), "deviceId", strlen("deviceId"),
                               reg_info->deviceId, sizeof(reg_info->deviceId));
...
static iot_agent_status_t formatRegistrationUsername(azure_registration_info_t *reg_info,
                                                     const char *idscope,
                                                     const char *deviceid)
{
    size_t n = snprintf(reg_info->username, sizeof(reg_info->username),
                        "%s/registrations/%s/api-version=2018-11-01&ClientVersion=1.4.0", idscope, deviceid);
...
{
    size_t m =
        snprintf(conn_info->username, sizeof(conn_info->username), "%s/%s/?api-version=2018-06-30", hubname, deviceid);
    size_t n = snprintf(conn_info->topic, sizeof(conn_info->topic), "devices/%s/messages/events/", deviceid);
...
static iot_agent_status_t azureRegister(const nxp_iot_ServiceDescriptor *service_descriptor,
                                        azure_registration_info_t *reg_info)
{
    ...
    while (retryCount < 5)
    {
        IOT_AGENT_INFO("\nMQTT attempting to register Azure Service '%s' (the operation migth take around 1 minute)...",
...
static void iot_agent_convert_id_to_label(char *label, size_t label_size, uint32_t id)
{
    ...
    uint32_t sss_keyId =
        (uint32_t)((((id >> (8 * 0)) & 0x000000FF) << (8 * 3)) | (((id >> (8 * 1)) & 0x000000FF) << (8 * 2)) |
                   (((id >> (8 * 2)) & 0x000000FF) << (8 * 1)) | (((id >> (8 * 3)) & 0x000000FF) << (8 * 0)));
