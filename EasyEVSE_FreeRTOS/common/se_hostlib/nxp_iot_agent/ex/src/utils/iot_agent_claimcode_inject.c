...
iot_agent_status_t iot_agent_claimcode_inject(ex_sss_boot_ctx_t *pCtx, char *claimcode, size_t claimcode_len)
{
    ...
    const sss_policy_u default_user = {
        .type = KPolicy_File, .auth_obj_id = DEFAULT_USER_ID, .policy = {.file = {.can_Write = 1U}}};
    const sss_policy_u default_user_common = {
        .type = KPolicy_Common, .auth_obj_id = DEFAULT_USER_ID, .policy = {.common = {.can_Delete = 1U}}};

    sss_policy_u edgelock2go_user_common = {.type        = KPolicy_Common,
                                            .auth_obj_id = 0U,
                                            .policy      = {.common = {
                                                                .can_Delete = 1U,
                                                       }}};
    sss_policy_u edgelock2go_user        = {
               .type = KPolicy_File, .auth_obj_id = 0U, .policy = {.file = {.can_Read = 1U, .can_Write = 1U}}};
    ...
    sss_status = sss_key_object_allocate_handle(&obj, CLAIMCODE_OBJ_ID, kSSS_KeyPart_Default, kSSS_CipherType_Binary,
                                                claimcode_len, (uint32_t)kKeyObject_Mode_Persistent);
    SSS_SUCCESS_OR_EXIT_MSG("sss_key_object_allocate_handle failed with 0x%04x", sss_status);

    sss_status = sss_key_store_set_key(&pCtx->ks, &obj, (uint8_t *)claimcode, claimcode_len, claimcode_len * 8U,
                                       &policy_for_claimcode, 0U);
