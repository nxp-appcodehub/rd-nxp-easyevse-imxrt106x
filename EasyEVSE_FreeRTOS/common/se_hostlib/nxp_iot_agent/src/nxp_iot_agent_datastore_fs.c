...
#define IOT_AGENT_DATASTORE_FS_VERSION                                                                          \
    ((uint32_t)(((IOT_AGENT_DATASTORE_FS_VERSION_MAJOR * 256U) + IOT_AGENT_DATASTORE_FS_VERSION_MINOR) * 256U + \
                IOT_AGENT_DATASTORE_FS_VERSION_PATCH))
...
iot_agent_status_t iot_agent_datastore_fs_open_file(FILE **fp, const char *basename, size_t index, const char *mode)
{
...
iot_agent_status_t iot_agent_datastore_fs_init(iot_agent_datastore_t *datastore,
                                               uint32_t identifier,
                                               const char *basename,
                                               iot_agent_datastore_validator_t validator)
{
    iot_agent_status_t agent_status            = IOT_AGENT_SUCCESS;
    ...
    agent_status                = iot_agent_datastore_fs_open_file(&datastore_context->fp[datastore_context->idx_read],
                                                                   datastore_context->basename, datastore_context->idx_read, "r+b");
    ...
    if (valid[0])
    {
        ...
        agent_status =
            iot_agent_datastore_fs_open_file(&datastore_context->fp[1], datastore_context->basename, 1U, "w+b");
    ...
    else if (valid[1])
    {
        ...
        agent_status =
            iot_agent_datastore_fs_open_file(&datastore_context->fp[0], datastore_context->basename, 0U, "w+b");
...
iot_agent_status_t iot_agent_datastore_fs_destroy(void *context)
{
...
iot_agent_status_t iot_agent_datastore_fs_allocate(void *context, size_t len)
{
    iot_agent_status_t agent_status                     = IOT_AGENT_SUCCESS;
    iot_agent_datastore_fs_context_t *datastore_context = (iot_agent_datastore_fs_context_t *)context;
    ...
    agent_status = iot_agent_datastore_fs_open_file(&datastore_context->fp[datastore_context->idx_write],
                                                    datastore_context->basename, datastore_context->idx_write, "w+b");
    ASSERT_OR_EXIT_STATUS_MSG(agent_status == IOT_AGENT_SUCCESS, IOT_AGENT_ERROR_FILE_SYSTEM,
                              "Unable to open file [%s.%u].", datastore_context->basename,
                              datastore_context->idx_write);

exit:
    return agent_status;
}

iot_agent_status_t iot_agent_datastore_fs_write(void *context, size_t offset, const void *src, size_t len)
{
    int fp_status                                       = 0;
    size_t written                                      = 0U;
    iot_agent_status_t agent_status                     = IOT_AGENT_SUCCESS;
...
iot_agent_status_t iot_agent_datastore_fs_read(void *context, void *dst, size_t offset, size_t *len)
{
    iot_agent_datastore_fs_context_t *datastore_context = (iot_agent_datastore_fs_context_t *)context;
...
iot_agent_status_t iot_agent_datastore_fs_commit(void *context)
{
    iot_agent_status_t agent_status                     = IOT_AGENT_SUCCESS;
    iot_agent_datastore_fs_context_t *datastore_context = (iot_agent_datastore_fs_context_t *)context;
    ...
    if (agent_status != IOT_AGENT_SUCCESS)
    {
        IOT_AGENT_ERROR("Unable to open file [%s.%u] with mode [%s]", datastore_context->basename,
                        datastore_context->idx_read, "w+b");
    }
...
bool iot_agent_datastore_fs_get_endpoint_info(void *context, void *endpoint_information)
{
                                           ...
                                           void *context)
{
    ...
    if (request.has_operation)
    {
        if (request.operation == nxp_iot_DatastoreOperation_READ)
        {
            ...
            agent_status =
                iot_agent_datastore_fs_read(context, response_buffer->pos, (size_t)request.offset, &read_data_len);
            ...
            result = encode_varint_with_fixed_length(
                read_data_len + (size_t)(data_length_buffer - datastore_response_length_buffer),
