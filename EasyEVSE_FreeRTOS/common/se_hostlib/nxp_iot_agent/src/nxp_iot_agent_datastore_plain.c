...
#define IOT_AGENT_DATASTORE_PLAIN_VERSION                                                                             \
    ((uint32_t)(((IOT_AGENT_DATASTORE_PLAIN_VERSION_MAJOR * 256U) + IOT_AGENT_DATASTORE_PLAIN_VERSION_MINOR) * 256U + \
                IOT_AGENT_DATASTORE_PLAIN_VERSION_PATCH))
...
iot_agent_status_t iot_agent_datastore_plain_init(iot_agent_datastore_t *datastore, uint32_t identifier)
{
...
iot_agent_status_t iot_agent_datastore_plain_destroy(void *context)
{
 ...
 *   @return agent_status
 */
iot_agent_status_t iot_agent_datastore_plain_allocate(void *context, size_t len)
{
    iot_agent_status_t agent_status                        = IOT_AGENT_SUCCESS;
    iot_agent_datastore_plain_context_t *datastore_context = (iot_agent_datastore_plain_context_t *)context;
    ...
    ASSERT_OR_EXIT_STATUS_MSG(datastore_context->buffers[datastore_context->idx_write] != NULL, IOT_AGENT_ERROR_MEMORY,
                              "Unable to allocate memory of size %u", len);
...
iot_agent_status_t iot_agent_datastore_plain_write(void *context, size_t offset, const void *src, size_t len)
{
    iot_agent_status_t agent_status                        = IOT_AGENT_SUCCESS;
    iot_agent_datastore_plain_context_t *datastore_context = (iot_agent_datastore_plain_context_t *)context;
    ...
    ASSERT_OR_EXIT_MSG(offset + len <= write_buffer_size,
                       "Writing out of bounds. Offset: %u, len: %u, write_buffer_size: %u.", offset, len,
                       write_buffer_size);
...
iot_agent_status_t iot_agent_datastore_plain_read(void *context, void *dst, size_t offset, size_t *len)
{
    iot_agent_status_t agent_status                        = IOT_AGENT_SUCCESS;
    iot_agent_datastore_plain_context_t *datastore_context = (iot_agent_datastore_plain_context_t *)context;
...
bool iot_agent_datastore_plain_get_endpoint_info(void *context, void *endpoint_information)
{
                                              ...
                                              void *context)
{
    ...
    if (request.has_operation)
    {
        if (request.operation == nxp_iot_DatastoreOperation_READ)
        {
            ...
            if ((size_t)request.length < read_data_len)
            ...
            agent_status = iot_agent_datastore_plain_read(datastore_context, response_buffer->pos,
                                                          (size_t)request.offset, &read_data_len);
