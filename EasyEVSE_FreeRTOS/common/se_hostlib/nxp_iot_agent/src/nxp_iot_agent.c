...
#define IOT_AGENT_VERSION (((IOT_AGENT_VERSION_MAJOR * 256) + IOT_AGENT_VERSION_MINOR) * 256 + IOT_AGENT_VERSION_PATCH)
...
iot_agent_status_t iot_agent_register_keystore(iot_agent_context_t *ctx, iot_agent_keystore_t *keystore)
{
    ...
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
                       "Unable to register keystore with id [0x%08lx], "
                       "a datastore with that id is already registered.",
                       keystore->identifier);
    agent_status = iot_agent_get_keystore_index_by_id(ctx, keystore->identifier, &index);
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
                       "Unable to register keystore with id [0x%08lx], "
                       "a keystore with that id is already registered.",
                       keystore->identifier);
#else
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
                       "Unable to register keystore with id [0x%08x], "
                       "a datastore with that id is already registered.",
                       keystore->identifier);
    agent_status = iot_agent_get_keystore_index_by_id(ctx, keystore->identifier, &index);
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
...
iot_agent_status_t iot_agent_register_datastore(iot_agent_context_t *ctx, iot_agent_datastore_t *datastore)
{
    ...
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
                       "Unable to register datastore with id [0x%08lx], "
                       "a datastore with that id is already registered.",
    ...
    agent_status = iot_agent_get_datastore_index_by_id(ctx, datastore->identifier, &index);
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
                       "Unable to register datastore with id [0x%08x], "
                       "a datastore with that id is already registered.",
                       datastore->identifier);
    agent_status = iot_agent_get_keystore_index_by_id(ctx, datastore->identifier, &index);
    ASSERT_OR_EXIT_MSG(agent_status != IOT_AGENT_SUCCESS,
    ...
    nxp_iot_UpdateStatusReport *status_report)
{
    ...
    ASSERT_OR_EXIT_MSG(service_descriptor->has_client_key_sss_ref,
                       "service_descriptor does not contain a client_key_sss_ref.");
    ASSERT_OR_EXIT_MSG(service_descriptor->client_key_sss_ref.has_type, "client_key_sss_ref does not contain a type.");
    ASSERT_OR_EXIT_MSG(service_descriptor->client_key_sss_ref.has_endpoint_id,
                       "client_key_sss_ref does not contain an endpoint_id.");
    ASSERT_OR_EXIT_MSG(service_descriptor->client_key_sss_ref.has_object_id,
                       "client_key_sss_ref does not contain an object_id.");

    // For the client certificate, we for now always expect to have it as a reference.
    ASSERT_OR_EXIT_MSG(service_descriptor->has_client_certificate_sss_ref,
                       "service_descriptor does not contain a client_certificate_sss_ref.");
    ASSERT_OR_EXIT_MSG(service_descriptor->client_certificate_sss_ref.has_type,
                       "client_certificate_sss_ref does not contain a type.");
    ASSERT_OR_EXIT_MSG(service_descriptor->client_certificate_sss_ref.has_endpoint_id,
                       "client_certificate_sss_ref does not contain an endpoint_id.");
    ASSERT_OR_EXIT_MSG(service_descriptor->client_certificate_sss_ref.has_object_id,
                       "client_certificate_sss_ref does not contain an object_id.");
    ...
    ASSERT_OR_EXIT_MSG(service_descriptor->server_certificate != NULL,
                       "service_descriptor does not contain a server certificate");
#endif
#if AX_EMBEDDED && defined(USE_RTOS) && USE_RTOS == 1
    IOT_AGENT_INFO("Updating device configuration from [%s]:[%lu].", service_descriptor->hostname,
                   service_descriptor->port);
#else
    IOT_AGENT_INFO("Updating device configuration from [%s]:[%d].", service_descriptor->hostname,
                   service_descriptor->port);
#endif
    ...
    sss_status =
        sss_key_object_get_handle(&client_certificate, service_descriptor->client_certificate_sss_ref.object_id);
    SSS_SUCCESS_OR_EXIT_MSG("sss_key_object_get_handle failed: 0x%08x.", sss_status);
...
#if NXP_IOT_AGENT_VERIFY_EDGELOCK_2GO_SERVER_CERTIFICATE
    bio_in_verify = BIO_new_mem_buf(service_descriptor->server_certificate->bytes,
                                    (int)service_descriptor->server_certificate->size);
    ...
    ASSERT_OR_EXIT_MSG(network_status == NETWORK_STATUS_OK,
                       "network_openssl_engine_session_disconnect failed with 0x%08x.", network_status);
    ...
    agent_status          = iot_agent_utils_gen_key_ref(sss_context, &private_key, private_key_object_id,
                                                        openssl_network_config.private_key);
    ...
    ASSERT_OR_EXIT_MSG(network_status == NETWORK_STATUS_OK,
                       "network_openssl_engine_session_connect failed with 0x%08x.", network_status);
    ...
    ASSERT_OR_EXIT_MSG(network_status == 0, "mbedtls_x509_crt_parse_der of client cert failed with 0x%08x",
                       network_status);
    ...
    while (pos < end)
    {
        ...
        size_t len           = 0;
        network_status =
            mbedtls_asn1_get_tag((unsigned char **)&pos, end, &len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
        ASSERT_OR_EXIT_MSG(network_status == 0, "mbedtls_asn1_get_tag of trusted root ca cert failed with 0x%08x",
                           network_status);
        pos += len;
        ...
        if (network_status == 0)
        {
            configPRINTF(("mbedtls_x509_crt_parse_der of trusted root ca success"));
        }
        ASSERT_OR_EXIT_MSG(network_status == 0, "mbedtls_x509_crt_parse_der of trusted root ca cert failed with 0x%08x",
                           network_status);
    }
#endif
    ...
    sss_mbedtls_associate_keypair(&((mbedtls_network_context_t *)dispatcher_context.network_context)->pkey,
                                  &private_key);
#endif
...
#if SSS_HAVE_HOSTCRYPTO_OPENSSL
    network_status = network_openssl_engine_session_disconnect(dispatcher_context.network_context);
    ASSERT_OR_EXIT_MSG(network_status == 0, "network_openssl_engine_session_disconnect failed with 0x%08x",
                       network_status);
#endif
...
exit:
    ...
    return (agent_status == IOT_AGENT_SUCCESS && (!dispatcher_context.update_status)) ? IOT_AGENT_UPDATE_FAILED :
                                                                                        agent_status;
}

iot_agent_status_t iot_agent_update_device_configuration_from_datastore(iot_agent_context_t *agent_context,
                                                                        ...
                                                                        nxp_iot_UpdateStatusReport *status_report)
{
    ...
    for (size_t i = 0U; i < number_of_services; i++)
    {
        ...
        if (service_descriptor.has_service_type &&
            (service_descriptor.service_type == nxp_iot_ServiceType_EL2GOSERVICE))
        {
            continue;
        }
        ...
        agent_status = iot_agent_update_device_configuration_from_service_descriptor(agent_context, &service_descriptor,
                                                                                     status_report);
...
iot_agent_status_t iot_agent_update_device_configuration_from_constants(iot_agent_context_t *agent_context,
                                                                        uint32_t client_key_object_id,
                                                                        uint32_t client_cert_object_id,
                                                                        nxp_iot_UpdateStatusReport *status_report)
{
    iot_agent_status_t agent_status = IOT_AGENT_SUCCESS;
    ...
    agent_status = iot_agent_update_device_configuration_from_service_descriptor(agent_context, &service_descriptor,
                                                                                 status_report);
    AGENT_SUCCESS_OR_EXIT_MSG("iot_agent_update_device_configuration_from_service_descriptor failed with 0x%08x.",
                              agent_status);
                                                         ...
                                                         nxp_iot_UpdateStatusReport *status_report)
{
    ...
    if (agent_status == IOT_AGENT_SUCCESS && configuration_valid)
    {
        ...
        if (agent_status != 0)
        {
            configPRINTF(("iot_agent_update_device_configuration_from_datastore failed with 0x%08x", agent_status));
            goto exit;
        }
        else
        {
            configPRINTF(("iot_agent_update_device_configuration_from_datastore success "));
        }
    }
    else
    {
        ...
        if (agent_status != 0)
        {
            configPRINTF(("iot_agent_get_keystore_by_id failed with 0x%08x", agent_status));
            goto exit;
        }
        ...
        agent_status = iot_agent_update_device_configuration_from_constants(agent_context, client_key_object_id,
                                                                            client_cert_object_id, status_report);
        AGENT_SUCCESS_OR_EXIT_MSG("iot_agent_update_device_configuration_from_constants failed with 0x%08x",
                                  agent_status);
    }
exit:
    return agent_status;
...
iot_agent_status_t iot_agent_select_service_by_index(iot_agent_context_t *ctx,
                                                     size_t index,
                                                     nxp_iot_ServiceDescriptor *service_descriptor)
{
    iot_agent_status_t agent_status = IOT_AGENT_SUCCESS;
    ...
    for (size_t i = 0U; i < ctx->numDatastores; i++)
    {
        ...
        if (remaining < numServices)
        {
            size_t offset;
            agent_status =
                iot_agent_service_get_service_offset_by_index(datastore, remaining, &offset, service_descriptor);
            AGENT_SUCCESS_OR_EXIT();
...
iot_agent_status_t iot_agent_select_service_by_id(iot_agent_context_t *ctx,
                                                  uint64_t service_id,
                                                  nxp_iot_ServiceDescriptor *service_descriptor)
{
...
iot_agent_status_t iot_agent_get_service_descriptor(const iot_agent_context_t *ctx,
                                                    nxp_iot_ServiceDescriptor *service_descriptor)
{
    return iot_agent_service_get_service_descriptor_of_service(ctx->selectedService.datastore,
                                                               ctx->selectedService.offset, service_descriptor);
}
...
iot_agent_status_t iot_agent_get_datastore_by_id(const iot_agent_context_t *ctx,
                                                 const uint32_t id,
                                                 iot_agent_datastore_t **datastore)
{
    size_t index                    = 0U;
...
iot_agent_status_t iot_agent_get_keystore_by_id(const iot_agent_context_t *ctx,
                                                const uint32_t id,
                              ...
                              void *context)
{
    ...
    if (message_type == nxp_iot_AgentGoodbyeRequest_fields)
    {
        ...
        {
            dispatcher_context->update_status =
                request.status.status == nxp_iot_UpdateStatusReport_UpdateStatus_SUCCESS;
            if (dispatcher_context->status_report != NULL)
            {
                ...
                memcpy(dispatcher_context->status_report, &request.status,
                       sizeof(*(dispatcher_context->status_report)));
    ...
    else if (message_type == nxp_iot_AgentCrlRequest_fields)
    {
        ...
        if (request.crl != NULL)
        {
            ...
            int network_status                            = network_verify_server_certificate(
                dispatcher_context->network_context, service_descriptor->server_certificate->bytes,
                (size_t)service_descriptor->server_certificate->size, request.crl->bytes, (size_t)request.crl->size,
                &response_crl.error);
            if (network_status != NETWORK_STATUS_OK)
            {
...
#if SSS_HAVE_HOSTCRYPTO_OPENSSL
        {
            int network_status = network_openssl_engine_session_disconnect(
                (openssl_network_context_t *)dispatcher_context->network_context);
            if (network_status != NETWORK_STATUS_OK)
            {
                IOT_AGENT_ERROR("network_openssl_engine_session_disconnect failed with 0x%08x.", network_status);
                return false;
...
iot_agent_status_t iot_agent_init_dispatcher(iot_agent_dispatcher_context_t *dispatcher_context,
                                             iot_agent_context_t *agent_context,
                                             nxp_iot_ServiceDescriptor *service_descriptor,
                                             nxp_iot_UpdateStatusReport *status_report)
{
    ...
    if (agent_context->edgelock2go_datastore != NULL)
    {
        ...
        dispatcher_context->endpoints[num_endpoints].endpoint_interface =
            agent_context->edgelock2go_datastore->iface.endpoint_interface;
    ...
    for (size_t i = 0U; i < agent_context->numDatastores; i++)
    {
        ...
        dispatcher_context->endpoints[num_endpoints].endpoint_interface =
            agent_context->datastores[i]->iface.endpoint_interface;
