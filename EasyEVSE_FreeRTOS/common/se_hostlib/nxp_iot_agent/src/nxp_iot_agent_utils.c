...
const uint8_t prime256v1_private_key_template[] = {0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                                   0xA5, 0xA6, 0xB5, 0xB6, 0xA5, 0xA6, 0xB5, 0xB6, 0x00, 0x00};
...
iot_agent_status_t iot_agent_utils_gen_key_ref_ecc(sss_key_store_t *keyStore,
                                                   ...
                                                   EVP_PKEY *pkey)
{
    ...
    *(buffer_ptr)   = (uint8_t)(keyObject->keyId >> 0) & 0xFFU;
    ...
    OPENSSL_ASSERT_OR_EXIT_STATUS(bn_priv != NULL, "BN_bin2bn", IOT_AGENT_ERROR_CRYPTO_ENGINE_FAILED);

    openssl_status = EC_KEY_set_private_key(ec_key, bn_priv);
    OPENSSL_SUCCESS_OR_EXIT_STATUS("EC_KEY_set_private_key", IOT_AGENT_ERROR_CRYPTO_ENGINE_FAILED);
...
iot_agent_status_t iot_agent_utils_gen_key_ref(sss_key_store_t *keyStore,
                                               sss_object_t *keyObject,
                                               ...
                                               EVP_PKEY *pkey)
{
    ...
    SSS_SUCCESS_OR_EXIT_MSG("sss_key_object_get_handle failed: 0x%04x [object id: 0x%04x].", sss_status,
                            keyObject->keyId);
...
iot_agent_status_t iot_agent_utils_write_key_ref_service_pem(const iot_agent_context_t *ctx, const char *filename)
{
    iot_agent_status_t agent_status = IOT_AGENT_SUCCESS;
    sss_status_t sss_status;
...
static iot_agent_status_t iot_agent_utils_get_keystore_from_service_descriptor(
    const iot_agent_context_t *ctx, const nxp_iot_ServiceDescriptor *service_descriptor, sss_key_store_t **sss_keystore)
{
    uint32_t keystore_id            = 0U;
    iot_agent_keystore_t *keystore  = NULL;
    iot_agent_status_t agent_status = IOT_AGENT_SUCCESS;
...
#if (SSS_HAVE_HOSTCRYPTO_OPENSSL)
iot_agent_status_t iot_agent_utils_write_certificate_pem_cos_over_rtp(
    iot_agent_context_t *ctx, const nxp_iot_ServiceDescriptor *service_descriptor, const char *filename)
{
                                                                  ...
                                                                  const char *filename)
{
    ...
    agent_status = iot_agent_utils_write_key_ref_pem(sss_keystore, &key,
                                                     service_descriptor->client_key_sss_ref.object_id, filename);
    AGENT_SUCCESS_OR_EXIT();
...
iot_agent_status_t iot_agent_utils_get_oid_value_in_subject(
                                                               ...
                                                               const pb_bytes_array_t *trusted_root_ca_certificates)
{
    ...
    success &=
        EVP_DigestUpdate(digest_context, &buffer[sizeof(header->checksum)], total_size - sizeof(header->checksum));
    ...
    failed |= mbedtls_sha256_update_ret(&digest_context, &buffer[sizeof(header->checksum)],
