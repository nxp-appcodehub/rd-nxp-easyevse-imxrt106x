    ...
    if (datastore != NULL)
    {
        if (datastore->iface.destroy != NULL)
        {
            ...
            if (agent_status != IOT_AGENT_SUCCESS)
            {
                ...
                IOT_AGENT_ERROR("Failed to destroy datastore [type: %lu, id: 0x%08lx]: 0x%08x", datastore->type,
                                datastore->identifier, agent_status);
#else
                IOT_AGENT_ERROR("Failed to destroy datastore [type: %d, id: 0x%08x]: 0x%08x", datastore->type,
                                datastore->identifier, agent_status);
#endif
            }
        }
...
iot_agent_status_t iot_agent_datastore_write(iot_agent_datastore_t *datastore,
                                             size_t offset,
                                             const void *src,
                                             size_t len)
{
...
bool datastore_read_callback(pb_istream_t *stream, uint8_t *buf, size_t count)
{
    ...
    size_t read  = count;
    agent_status = datastore_stream_context->datastore->iface.read(datastore_stream_context->datastore->context, buf,
                                                                   datastore_stream_context->offset, &read);
    datastore_stream_context->offset += read;

    AGENT_SUCCESS_OR_EXIT_MSG("Read from datastore failed with 0x%04x", agent_status);

    ASSERT_OR_EXIT_MSG(read == count, "Needed to read %u bytes, but only %u bytes could be read.", count, read);
