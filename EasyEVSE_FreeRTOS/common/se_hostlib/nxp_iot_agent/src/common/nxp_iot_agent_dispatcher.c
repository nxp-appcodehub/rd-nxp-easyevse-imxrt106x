...
#define IOT_AGENT_DISPATCHER_VERSION                                                                        \
    ((uint32_t)(((IOT_AGENT_DISPATCHER_VERSION_MAJOR * 256U) + IOT_AGENT_DISPATCHER_VERSION_MINOR) * 256U + \
                IOT_AGENT_DISPATCHER_VERSION_PATCH))

static iot_agent_endpoint_cache_table_entry_t *find_endpoint_in_cache(iot_agent_endpoint_cache_table_entry_t *cache,
...
bool handle_request_payload(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
    ...
    if (message_type == nxp_iot_AgentHelloRequest_fields)
    {
        ...
        for (size_t i = 0U; i < NXP_IOT_AGENT_MAX_NUM_ENDPOINTS; i++)
        {
            ...
            nxp_iot_EndpointInformation *info =
                &response.message.hello.endpoints[response.message.hello.endpoints_count];
            info->has_type       = true;
    ...
    else
    {
        ...
        iot_agent_endpoint_cache_table_entry_t *endpoint =
            find_endpoint_in_cache(dispatcher_context->endpoints, type, endpointId);
        if (endpoint == NULL)
        {
...
iot_agent_status_t iot_agent_dispatcher(iot_agent_dispatcher_context_t *dispatcher_context,
                                        pb_istream_t *input,
                                        pb_ostream_t *output)
{
    ...
    COMPILE_TIME_ASSERT(NXP_IOT_AGENT_MAX_NUM_ENDPOINTS <= (sizeof(((nxp_iot_AgentHelloResponse *)0)->endpoints) /
                                                            sizeof(((nxp_iot_AgentHelloResponse *)0)->endpoints[0])));
    ...
    COMPILE_TIME_ASSERT(NXP_IOT_AGENT_CORRELATION_ID_LENGTH ==
                        sizeof(((nxp_iot_UpdateStatusReport *)0)->correlationId));
