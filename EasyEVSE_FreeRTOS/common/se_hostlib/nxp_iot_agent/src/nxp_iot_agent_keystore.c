    ...
    if (keystore != NULL)
    {
        if (keystore->iface.destroy != NULL)
        {
            ...
            if (agent_status != IOT_AGENT_SUCCESS)
            {
                ...
                IOT_AGENT_ERROR("Failed to destroy keystore [type: %lu, id: 0x%08lx]: 0x%08x", keystore->type,
                                keystore->identifier, agent_status);
#else
                IOT_AGENT_ERROR("Failed to destroy keystore [type: %d, id: 0x%08x]: 0x%08x", keystore->type,
                                keystore->identifier, agent_status);
#endif
            }
        }
