...
bool iot_agent_service_is_configuration_data_valid(const iot_agent_datastore_t *ctx)
{
    ...
    ASSERT_OR_EXIT_MSG(header.length >= sizeof(header),
                       "Length indicated in header [%lu] is too small to contain configuration data.", header.length);
...
#else
    ASSERT_OR_EXIT_MSG(header.length >= sizeof(header),
                       "Length indicated in header [%d] is too small to contain configuration data.", header.length);
 ...
 *   @return agent_status
 */
iot_agent_status_t iot_agent_service_get_service_offset_by_id(const iot_agent_datastore_t *datastore,
                                                              uint64_t service_id,
                                                              size_t *offset,
                                                              nxp_iot_ServiceDescriptor *service_descriptor)
{
