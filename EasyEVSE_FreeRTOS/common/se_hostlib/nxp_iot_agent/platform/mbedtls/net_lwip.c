    ...
    mbedtls_net_context *bind_ctx, mbedtls_net_context *client_ctx, void *client_ip, size_t buf_size, size_t *ip_len)
{
    ...
    if (getsockopt(bind_ctx->fd, SOL_SOCKET, SO_TYPE, (void *)&type, (socklen_t *)&type_len) != 0 ||
    ...
    else
    {
        ...
        ret = recvfrom(bind_ctx->fd, buf, sizeof(buf), MSG_PEEK, (struct sockaddr *)&client_addr, &n);
    ...
    if (type != SOCK_STREAM)
    {
        ...
        if (getsockname(client_ctx->fd, (struct sockaddr *)&local_addr, &n) != 0 ||
            (bind_ctx->fd = (int)socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0 ||
