...
static const mbedtls_ecp_group_id supported_curves[] = {
...
void *network_new()
{
    mbedtls_network_context_t *network_ctx =
        (mbedtls_network_context_t *)NETWORK_malloc(sizeof(mbedtls_network_context_t));
    if (network_ctx != NULL)
    {
...
int network_configure(void *opaque_ctx, void *opaque_network_config)
{
    ...
    if ((ret = mbedtls_ctr_drbg_seed(&(network_ctx->ctr_drbg), mbedtls_entropy_func, &(network_ctx->entropy),
...
int network_connect(void *opaque_ctx)
{
    ...
    if ((ret = mbedtls_net_connect(&network_context->server_fd, network_config->hostname, port_str,
    ...
    if ((ret = mbedtls_ssl_config_defaults(&(network_context->conf), MBEDTLS_SSL_IS_CLIENT,
...
#error Too big buffer for TLS records. You are wasting memory.
    ...
    if ((ret = mbedtls_ssl_conf_own_cert(&(network_context->conf), &network_context->network_config.clicert,
                                         &(network_context->pkey))) != 0)
    {
        return -1;
    }

    if ((ret = mbedtls_ssl_setup(&(network_context->ssl), &(network_context->conf))) != 0)
    {
        return -1;
    }
    ...
    mbedtls_ssl_set_bio(&network_context->ssl, &network_context->server_fd, mbedtls_net_send, mbedtls_net_recv, NULL);
...
int network_verify_server_certificate(
    void *context, uint8_t *trusted_bytes, size_t trusted_size, uint8_t *crl_bytes, size_t crl_size, uint32_t *error)
{
    ...
    *error = 0;
    network_status =
        mbedtls_x509_crt_verify(ssl->session->peer_cert, network_context->conf.ca_chain, &crl, NULL, error, NULL, NULL);
    if (*error != 0)
    {
