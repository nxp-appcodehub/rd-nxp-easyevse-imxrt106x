...
static vehicle_auth_methods_t vehicle_auth_method = VehicleAuth_EIM;
static V2G_status_t V2G_status                    = DEMO;

static evse_iso15118_state_t s_currentState = EVSE_ISO15118_NotEnable;

const char* iso15118StateToString[EVSE_ISO15118_LAST] =
{
        [EVSE_ISO15118_NotEnable]   = "ISO15118 not activated",
        [EVSE_ISO15118_InitStack]   = "ISO15118 init",
        [EVSE_ISO15118_FWDownload]  = "Firmware download",
        [EVSE_ISO15118_WaitConnect] = "Wait car",
        [EVSE_ISO15118_SLACStart]   = "SLAC in progress",
        [EVSE_ISO15118_SLACFailed]  = "SLAC failed",
        [EVSE_ISO15118_SLACSucces]  = "SLAC Success",
        [EVSE_ISO15118_AuthWait]    = "Wait for auth",
        [EVSE_ISO15118_AuthFailed]  = "Auth failed",
        [EVSE_ISO15118_V2GRunnig]   = "Demo running",
};
...
{
    if ((s_currentState == EVSE_ISO15118_V2GRunnig) && (state != s_currentState))
    {
        EVSE_ISO15118_SetV2GStatus(NOT_CONNECTED);
    }

    if ((state < EVSE_ISO15118_LAST) && (s_currentState != state))
    {
...
void  EVSE_ISO15118_GetVehicleData(vehicle_data_t *vehicle_data)
{
    ...
    vehicle_data->energyDeliveryStatus = getEnergyDeliveryStatus(vehicle_data->requestedEnergy, vehicle_data->deliveredEnergyHLC);
}
