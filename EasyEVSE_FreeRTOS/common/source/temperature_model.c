...
float power_radiation()
{
    return STEFAN_BOLTZMANN * EMISSIVITY * SURFACE_AREA *
           ((T * T) * (T * T) -
            (AMBIENT_TEMPERATURE * AMBIENT_TEMPERATURE) * (AMBIENT_TEMPERATURE * AMBIENT_TEMPERATURE));
}
...
{
    /* TODO */
    //	T += (power_electricity(message_irms, message_vrms) - power_convection() - power_radiation()) / HEAT_CAPACITY *
    // dt; 	evse_temperature_C = T - KELVIN;
}
