...
typedef struct _file_meta
{
    ...
    file_encrypt_info_t encryptInfo;
} file_meta_t;
...
static const encrypt_ctx_t s_flashLittlefsEncCtx = {
...
static flash_fs_cbs_t s_flashLittlefsCbs = {NULL};
...
static bool _lfs_checkBlockEmpty(lfs_block_t block)
{
    ...
    {
        uint32_t dst = 0;
        mflash_drv_read((src + i), (uint32_t *)&dst, sizeof(uint32_t));
...
static int LFS_FlashRead(const struct lfs_config *lfsc, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    mflash_drv_read(src, (uint32_t *)dst, size);
    ...
    const struct lfs_config *lfsc, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size)
{
    ...
    {
        status = mflash_drv_page_program(prog_addr + pos, (uint32_t *)((uintptr_t)buffer + pos));
...
static int LFS_FlashErase(const struct lfs_config *lfsc, lfs_block_t block)
{
    ...
    if (!_lfs_checkBlockEmpty(block))
    {
        ...
        status = mflash_drv_sector_erase(erase_addr);
...
static flash_fs_status_t LFS_SaveFileContent(file_meta_t *file_meta, uint8_t *dataIn, uint32_t dataLenIn)
{
    ...
    {
        int32_t status;
        len  = Encrypt_Get_Crypt_Length(dataLenIn);
        data = (uint8_t *)pvPortMalloc(len);
        if (data == NULL)
        {
            ret = FLASH_FS_ENOMEM3;
        }
        else
        {
                                                ...
                                                uint32_t *dataLenOut)
{
...
#endif /* if FS_ENCRYPTION_SUPPORTED */
    {
        data = dataOut;
        len  = *dataLenOut;
    ...
    if (ret == FLASH_FS_OK)
    {
        ...
        if (file_meta->encryptInfo.useEncryption)
        {
            int32_t status = Decrypt_AES_CBC_PKCS7(&s_flashLittlefsEncCtx, data, len, dataPlain, &len);
            if (status == ENCRYPT_STATUS_OK)
            {
                if (len <= offset)
                {
...
flash_fs_status_t FLASH_LITTLEFS_Init(uint8_t erase)
{
    ...
    if (FLASH_FS_OK == ret)
    {
        ...
        {
#if FS_ENCRYPTION_SUPPORTED
            Encrypt_Init_Slot(&s_flashLittlefsEncCtx);
#endif /* FS_ENCRYPTION_SUPPORTED */
        }
    }
...
flash_fs_status_t FLASH_LITTLEFS_Mkfile(const char *name, bool encrypt)
{
    ...
    if ((ret == FLASH_FS_OK) || (ret == FLASH_FS_FILEEXIST))
    {
        lfs_file_close(&s_LittlefsHandler.lfs, &file_meta.file);
    }
