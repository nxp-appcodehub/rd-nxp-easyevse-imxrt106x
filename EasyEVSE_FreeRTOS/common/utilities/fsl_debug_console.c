...
SemaphoreHandle_t xMutexDbgConsole = NULL;
StaticSemaphore_t xMutexDbgConsoleBuffer;
...
status_t DbgConsole_Init(uint8_t instance, uint32_t baudRate, serial_port_type_t device, uint32_t clkSrcFreq)
{
    ...
    DEBUG_CONSOLE_CREATE_MUTEX_SEMAPHORE(xMutexDbgConsole, &xMutexDbgConsoleBuffer);
...
int DbgConsole_Printf(const char *fmt_s, ...)
{
    ...
    int result = 0;
#if (EASYEVSE == 1)
    DEBUG_CONSOLE_TAKE_MUTEX_SEMAPHORE_BLOCKING(xMutexDbgConsole);
#endif
    va_start(ap, fmt_s);
    result = DbgConsole_Vprintf(fmt_s, ap);
    va_end(ap);
#if (EASYEVSE == 1)
    DEBUG_CONSOLE_GIVE_MUTEX_SEMAPHORE(xMutexDbgConsole);
#endif /* (EASYEVSE == 1) */
    return result;
}
