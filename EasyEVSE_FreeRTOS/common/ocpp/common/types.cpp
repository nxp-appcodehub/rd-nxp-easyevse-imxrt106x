...
#else
DateTimeImpl::DateTimeImpl(OcppTimepoint timepoint) : timepoint(timepoint)
{
}
#endif
...
std::string DateTimeImpl::to_rfc3339() const {
    ...
    int n = snprintf(buff, sizeof(buff),
                     ...
                     this->timepoint.year,
                     this->timepoint.month,
                     this->timepoint.day,
                     this->timepoint.hours,
                     this->timepoint.minutes,
                     this->timepoint.seconds);


    // Safety check: ensure snprintf didn't overflow
    if (n < 0 || n >= static_cast<int>(sizeof(buff))) {
        configPRINTF(("to_rfc3339: formatting error or overflow"));
        return std::string(); // return empty string on error
    }

    return std::string(buff);
    //return "2025-09-11T12:21:00Z";    //"LoveIsAllYouNeed";
#endif
}
...
OcppTimepoint DateTimeImpl::to_time_point() const {
    return this->timepoint;
}

unsigned long int DateTimeImpl::to_time_point_s() const {
    return this->timepoint.timestamp_s;
}
...
void from_json(const json& j, RunningCost& c) {
    ...
    if (j.contains("priceTextExtra")) {
        ...
        if (!price_text.is_array()) {
            configPRINTF(("priceTextExtra should be an array, but is not. Content: %s ", price_text));
        } else {
        ...
        if (j.contains("atCPStatus")) {
            ...
            for (const auto& cp_status : j.at("atCPStatus").items()) {
                try {
                    ...
                    configPRINTF(("Could not trigger on CP status: status %s is not a valid chargepoint status", cp_status.value().get<std::string>()));
                }
            }
