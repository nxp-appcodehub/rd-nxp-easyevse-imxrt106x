...
void MessageLogging::initialize() {
    if (this->log_messages) {
        if (this->rotate_logs) {
            configPRINTF(("Log rotation enabled"));
        }
        if (this->log_to_console) {
            configPRINTF(("Logging OCPP messages to console"));
        }
        if (this->message_callback != nullptr) {
            configPRINTF(("Logging OCPP messages to callback"));
        }
        if (this->log_to_file) {
            auto output_file_path = message_log_path + "/";
            output_file_path += output_file_name;
            output_file_path += +".log";
            configPRINTF(("Logging OCPP messages to log file: %s ", output_file_path));
        ...
        if (this->log_to_html) {
            ...
            html_file_path += ".html";
            configPRINTF(("Logging OCPP messages to html file: %s", html_file_path));
        ...
        if (this->log_security) {
            ...
            configPRINTF(("Logging SecurityEvents to file: %s", security_file_path));
        ...
        //sys();
        configPRINTF(("Session logging started."));
    }
}
...
LogRotationStatus MessageLogging::rotate_log(const std::string& file_basename) {
    ...
    if (this->maximum_file_count > 0 and files.size() >= this->maximum_file_count) {
        ...
        configPRINTF(("Removing oldest log file: %s", files.front()));
    ...
    for (auto& file : files) {
        if (file.filename() == file_basename) {
            ...
            configPRINTF(("Renaming: %s -> %s", file.string(), new_file_name.string()));
            ...
            try {
                ...
                configPRINTF(("Could not rename logfile: %s", file.string()));
            }
        }
    }
    return status;
...
LogRotationStatus MessageLogging::rotate_log_if_needed(const std::filesystem::path& path, std::ofstream& os,
    ...
    if (log_file_size >= maximum_file_size_bytes) {
        configPRINTF(("Logfile: %s file size %d byes > % bytes rotating log. ", path.filename().string(), log_file_size,
...
void MessageLogging::charge_point(const std::string& message_type, const std::string& json_str) {
	configPRINTF(("\r\n In MessageLogging::charge_point \r\n"));
    if (this->message_callback != nullptr) {
...
#else
        configPRINTF(("lock needed and not implemented in ocpp_logging.cpp"));
#endif
    ...
    if (this->log_messages) {
        ...
        if (typ == 0) {
            ...
            if (this->detailed_log_to_console) {
                configPRINTF(("\033[1;35mChargePoint: %s \033[1;0m", json_str));
            } else if (this->log_to_console) {
                configPRINTF(("\033[1;35mChargePoint: %s \033[1;0m", message_type));
            }
        } else if (typ == 1) {
            origin = "CentralSystem";
            target = "ChargePoint";
            if (this->detailed_log_to_console) {
                configPRINTF(("\033[1;36mCentralSystem: %\033[1;0m"));//, json_ptr));
            } else if (this->log_to_console) {
                configPRINTF(("                                    \033[1;36mCentralSystem: %s \033[1;0m", message_type));
            }
        } else {
            origin = "SYS";
            target = "";
            if (this->detailed_log_to_console || this->log_to_console) {
                configPRINTF(("\033[1;32mSYS:  %s \033[1;0m", message_type));
            }
        }
...
std::string MessageLogging::html_encode(const std::string& msg) {
    ...
    boost::replace_all(out, ">", "&gt;");
#else
    configPRINTF(("html_encode() not fully implemented."));
#endif
    return out;
}
...
FormattedMessageWithType MessageLogging::format_message(const std::string& message_type, const std::string& json_str) {
    ...
    try {
        ...
        configPRINTF(("Error parsing OCPP message %s : %s ", message_type, e.what()));
    }
