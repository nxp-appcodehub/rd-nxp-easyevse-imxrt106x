...
//EVSE
DatabaseHandlerCommon::DatabaseHandlerCommon(const fs::path& sql_migration_files_path,
                                             uint32_t target_schema_version) noexcept :
    sql_migration_files_path(sql_migration_files_path),
    target_schema_version(target_schema_version) {
}
...
std::vector<DBTransactionMessage> DatabaseHandlerCommon::get_message_queue_messages(const QueueType queue_type) {
    ...
    while ((status = stmt->step()) == SQLITE_ROW) {
        try {
            ...
            configPRINTF(("json parse failed because: (%s) ", e.what()));
        } catch (const std::exception& e) {
            configPRINTF(("can not get queued transaction message from database: (%s)", e.what()));
        }
    }

    if (status != SQLITE_DONE) {
        configPRINTF(("Could not get (all) queued transaction messages from database"));
