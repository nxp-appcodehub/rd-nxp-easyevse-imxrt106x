...
#include "freertos_recursive_mutex.hpp"
        ...
        if (unit_of_measure.unit.has_value()) {
            if (unit_of_measure.unit.value() == "kWh") {
                ...
                // do nothing
                    ...
                    throw std::runtime_error("Attempt to convert an energy value which does not have a correct unit");
            }
        }
...
bool Evse::does_connector_exist(const CiString<20> connector_type) const {
    ...
    for (uint32_t i = 1; i <= number_of_connectors; ++i) {
        ...
        try {
            ...
            configPRINTF(("Connector with id %d does not exist", i));
            continue;
        }
...
void Evse::try_resume_transaction() {
    ...
    if (this->id_connector_map.count(transaction->connector_id) != 0) {
        ...
        configPRINTF(("Can't resume transaction on evse_id %d  for non existent connector %d", evse_id,
                   transaction->connector_id));

        try {
            this->database_handler->transaction_delete(transaction->transactionId);
        } catch (const QueryExecutionException& e) {
            configPRINTF(("Can't delete transaction: "));// << e.what();
        }
...
void Evse::open_transaction(const std::string& transaction_id, const int32_t connector_id, const DateTime& timestamp,
    ...
    if (!this->id_connector_map.count(connector_id)) {
        throw std::runtime_error("Attempt to start transaction at invalid connector_id");
    }
    ...
    bool tx_database_enabled =
        this->device_model.get_optional_value<bool>(ControllerComponentVariables::ResumeTransactionsOnBoot)
            .value_or(false);
#else
    bool tx_database_enabled = false;
#endif
...
#else
    this->transaction->active_energy_import_start_value = 10.2f;
#endif
    ...
    try {
        ...
        configPRINTF(("Could not insert transaction meter values of transaction: %d into database ",
                      this->transaction->transactionId.get())); //" into database: " << e.what();
    } catch (const std::invalid_argument& e) {
        configPRINTF(("Could not insert transaction meter values of transaction: %d into database ",
                      this->transaction->transactionId.get()));
    }
...
void Evse::close_transaction(const DateTime& timestamp, const MeterValue& meter_stop, const ReasonEnum& reason) {
    if (this->transaction == nullptr) {
        configPRINTF(("Received attempt to stop a transaction without an active transaction"));
        return;
    }
...
void Evse::start_checking_max_energy_on_invalid_id() {
    if (this->transaction != nullptr) {
        ...
        configPRINTF(("Trying to start \"MaxEnergyOnInvalidId\" checking without an active transaction"));
    }
}
...
bool Evse::has_active_transaction(int32_t connector_id) const {
    if (!this->id_connector_map.count(connector_id)) {
        configPRINTF(("has_active_transaction called for invalid connector_id"));
        return false;
    }
...
void Evse::release_transaction() {
    try {
        ...
        configPRINTF(("Could not clear transaction meter values: "));
    }
...
void Evse::on_meter_value(const MeterValue& meter_value) {
	configPRINTF(("Inside Evse::on_meter_value"));
    ...
    this->check_max_energy_on_invalid_id();
    configPRINTF(("sending_meter_value_on_pricing_trigger..."));
...
#else
    int32_t max_energy_on_invalid_id = 500;
#endif
    auto& transaction = this->transaction;
    if (transaction != nullptr and max_energy_on_invalid_id and
        ...
        if (opt_energy_value.has_value() and active_energy_import_start_value.has_value()) {
            ...
            if (std::lround(charged_energy) >= max_energy_on_invalid_id) {
...
void Evse::start_metering_timers(const DateTime& timestamp) {
	configPRINTF(("Start metering timers\r\n"));
	configPRINTF(("timestamp when starting metering timers is: %s", timestamp.to_rfc3339().c_str()));

	configPRINTF(("timepoint set as start_time is %ld", timestamp.to_time_point().to_time_point_s()));

	/* default value for SampledDataTxUpdatedInterval is 120 seconds */
    const auto sampled_data_tx_updated_interval = std::chrono::seconds(10);
        //this->device_model.get_value<int>(ControllerComponentVariables::SampledDataTxUpdatedInterval));
    configPRINTF(("sampled_data_tx_updated_interval is %d seconds", static_cast<int>(sampled_data_tx_updated_interval.count())));
    if (sampled_data_tx_updated_interval > 0s) {
        this->transaction->sampled_tx_updated_meter_values_timer.interval_starting_from(
            [this]
			{
        		this->transaction_meter_value_req(this->get_meter_value(), *this->transaction);
            },
            sampled_data_tx_updated_interval,
			timestamp.to_time_point()); //date::utc_clock::to_sys(timestamp.to_time_point()));
    }
}
...
void Evse::set_meter_value_pricing_triggers(
    ...
    configPRINTF(("Set metervalue pricing triggers: "));
    trigger_metervalue_at_time.has_value() ?
                        configPRINTF(("at time: "))// , trigger_metervalue_at_time.value().to_rfc3339()));
                        : configPRINTF(("no time pricing trigger"));
    trigger_metervalue_on_energy_kwh.has_value() ?
                        configPRINTF(("on energy kWh: %d", trigger_metervalue_on_energy_kwh.value()))
                        : configPRINTF(("No energy kWh trigger"));
    trigger_metervalue_on_power_kw.has_value() ?
    		            configPRINTF(( "on power kW: %d", trigger_metervalue_on_power_kw.value()))
                        : configPRINTF(("No power kW trigger"));
    ...
    configPRINTF(("Could not set trigger metervalue because it is not implemented."));
#if 0
    ...
    if (trigger_timepoint < now) {
        configPRINTF(("Could not set trigger metervalue because trigger time is in the past."));
        return;
    }
    ...
    this->trigger_metervalue_at_time_timer = std::make_unique<Everest::SystemTimer>(&io_context, [this]() {
        configPRINTF(("Sending metervalue in timer"));
        ...
        if (meter_value.sampledValue.empty()) {
            configPRINTF(("Send latest meter value because of chargepoint time trigger failed"));
        } else {
    ...
    configPRINTF(("Set trigger metervalue at time "));// << trigger_timepoint;
...
void Evse::send_meter_value_on_pricing_trigger(const MeterValue& meter_value) {
    ...
    if (this->trigger_metervalue_on_energy_kwh.has_value()) {
        ...
        if (this->send_metervalue_function == nullptr) {
            configPRINTF(("Cost and price metervalue kwh trigger: Can not send metervalue because the send metervalue function is not set."));
            ...
            if (active_import_register_meter_value_wh.has_value() and
                ...
                if (active_import_meter_value.sampledValue.empty()) {
                    configPRINTF(("No current active import register metervalue found. Can not send trigger metervalue."));
                } else {
    ...
    if (this->send_metervalue_function == nullptr) {
        configPRINTF(("Cost and price metervalue wh trigger: Can not send metervalue because the send metervalue function is not set."));
...
Connector* Evse::get_connector(int32_t connector_id) const {
    if (connector_id <= 0 or connector_id > this->get_number_of_connectors()) {
        ...
        configPRINTF(("ConnectorID %d out of bounds for EVSE %d", connector_id, this->evse_id));
