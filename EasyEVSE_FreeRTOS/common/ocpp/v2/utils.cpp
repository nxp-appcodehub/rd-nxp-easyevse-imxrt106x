...
int EVP_Digest(const void *data, size_t count, unsigned char *md, unsigned int *size, unsigned int alg_type, uint32_t *implementation_engine)
{
	return 0;
}
...
std::vector<MeasurandEnum> get_measurands_vec(const std::string& measurands_csv) {
    ...
    for (const auto& measurand_string : measurands_strings) {
        try {
            ...
            configPRINTF(("Could not convert string: %s to MeasurandEnum", measurand_string));
        }
    }
    return measurands;
}
...
std::string sha256(const std::string& str) {
    ...
    EVP_Digest(str.c_str(), str.size(), hash, NULL, EVP_SHA_256, NULL);
    std::stringstream ss;
...
ocpp::DateTime align_timestamp(const DateTime timestamp, std::chrono::seconds align_interval) {
    if (align_interval.count() < 0) {
        configPRINTF(("Invalid align interval value"));
        return timestamp;
    }
    ...
    configPRINTF(("Original Timpestamp"));
#if 0
    configPRINTF(("Original Timestamp: " << timestamp.to_rfc3339() << std::endl));
    configPRINTF(("Interval: " << align_interval.count() << std::endl));
    configPRINTF(("Rounded Timestamp: " << rounded_time));
#endif

    return rounded_time;
#else
    configPRINTF(("align_timestamp not implemented"));
...
std::vector<ChargingProfilePurposeEnum> get_purposes_to_ignore(const std::string& csl, const bool is_offline) {
    ...
    for (const auto& purpose : purposes_to_ignore_vec) {
        try {
            ...
            configPRINTF(("Error while converting charging profile purpose to ignore: %s", purpose));
        }
    }
...
std::vector<OcppProtocolVersion> get_ocpp_protocol_versions(const std::string& csl) {
    ...
    for (const auto ocpp_version_str : ocpp_versions_str) {
        try {
            ...
            configPRINTF(("Error while converting ocpp protocol version: %s ", ocpp_version_str));
        }
    }
