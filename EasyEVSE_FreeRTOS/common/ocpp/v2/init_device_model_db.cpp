...
const static uint32_t MIGRATION_DEVICE_MODEL_FILE_VERSION_V2 = 5;
    ...
    if (delete_db_if_exists) {
        if (std::filesystem::exists(database_path)) {
            if (!std::filesystem::remove(database_path)) {
                throw InitDeviceModelDbError("Could not remove database " + database_path.string());
            }
...
void InitDeviceModelDb::insert_component(const ComponentKey& component_key,
    ...
    configPRINTF(("Inserting component %s - not implemented\r\n", component_key.name));
#if 0
    ...
    for (const auto& variable : component_variables) {
        configPRINTF(("-- Inserting variable %s", variable.name));
    ...
    for (const auto& path : components_config_path) {
        ...
        try {
            ...
            if (data.contains("properties")) {
                ...
                configPRINTF(("Component %s does not contain any properties", data.at("name")));
                continue;
            }
        } catch (const json::parse_error& e) {
            configPRINTF(("Error while parsing config file: %s", path));
            throw;
        }
    }
...
void InitDeviceModelDb::update_variable(const DeviceModelVariable& variable, const DeviceModelVariable& db_variable,
    ...
    if (!db_variable.db_id.has_value()) {
        configPRINTF(("Can not update variable %s : database id unknown", variable.name));
        return;
    }
...
void InitDeviceModelDb::delete_variable(const DeviceModelVariable& variable) {
    ...
    if (!variable.db_id.has_value()) {
        configPRINTF(("Can not remove variable %s from db: id unknown", variable.name));
        return;
    }
    ...
    if (delete_statement->step() != SQLITE_DONE) {
        configPRINTF(("Can not remove variable %s from db: %s", variable.name,
...
void InitDeviceModelDb::update_attribute(const VariableAttribute& attribute, const DbVariableAttribute& db_attribute,
    ...
    if (!db_attribute.db_id.has_value()) {
        configPRINTF(("Can not update attribute: id not found"));
        return;
    }
    ...
    if (update_statement->step() != SQLITE_DONE) {
        configPRINTF(("Could not update variable attribute: %s", this->database->get_error_message()));
    ...
    if (has_attribute_actual_value(attribute, default_actual_value)) {
        if (!insert_variable_attribute_value(
            ...
            configPRINTF(("Can not update variable attribute (%d value: %d)", db_attribute.db_id.value(),
...
void InitDeviceModelDb::delete_attribute(const DbVariableAttribute& attribute) {
    ...
    if (!attribute.db_id.has_value()) {
        configPRINTF(("Could not remove attribute, because id is unknown."));
        return;
    }
...
bool InitDeviceModelDb::insert_variable_attribute_value(const int64_t& variable_attribute_id,
    ...
    if (insert_variable_attribute_statement->step() != SQLITE_DONE) {
        ...
        configPRINTF(("Could not set value of variable attribute id %d value has already changed by other source ",
    ...
    while ((status = select_statement->step()) == SQLITE_ROW) {
        ...
        if (select_statement->column_type(22) != SQLITE_NULL) {
            try {
                ...
                configPRINTF((": Variable Source will not be set (so default will be used)"));
            }
        }
...
bool InitDeviceModelDb::remove_component_from_db(const ComponentKey& component) {
    ...
    if (!component.db_id.has_value()) {
        configPRINTF(("Can not delete component %s: no id given", component.name));
        return false;
    }
...
void InitDeviceModelDb::update_component_variables(
    ...
    if (!db_component.db_id.has_value()) {
        configPRINTF(("Can not update component %s  because database id is unknown.", db_component.name));
        return;
    }
...
static void check_integrity(const std::map<ComponentKey, std::vector<DeviceModelVariable>>& component_configs) {
    ...
    if (has_error) {
        throw InitDeviceModelDbError(final_error_message);
    }
}
...
static std::string get_string_value_from_json(const json& value) {
    if (value.is_string()) {
        ...
        configPRINTF(("String value %s from config is an object or array, but config values should be from a primitive type.",value.dump()));
        return value.dump();
    } else {
        return value.dump();
    }
