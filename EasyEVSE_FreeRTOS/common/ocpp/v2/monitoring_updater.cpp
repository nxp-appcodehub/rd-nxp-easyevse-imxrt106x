    ...
    if constexpr (T == DataEnum::boolean) {
        ...
        if (monitor_meta.monitor.type == MonitorEnum::Delta) {
            if (monitor_meta.reference_value.has_value()) {
                ...
                configPRINTF(("Invalid reference value for monitor ")); // << monitor_meta.monitor;
                return false;
            }
            ...
            configPRINTF(("Requested unsupported trigger monitor of type: %d", monitor_meta.monitor.type));
            return false;
        }
    }
...
#if 0
    configPRINTF(("Aligned time: " << std::ctime(&dbg_time_now) << " with interval: " << monitor_seconds.count()
...
EventData create_notify_event(int32_t unique_id, const std::string& reported_value, const Component& component,
    ...
    if (monitor_meta.monitor.type == MonitorEnum::Periodic ||
        ...
        configPRINTF(("Invalid monitor type of: %d", monitor_meta.monitor.type));
    }
    ...
    if (monitor_meta.type == VariableMonitorType::HardWiredMonitor) {
        ...
        configPRINTF(("Invalid monitor meta type of: %d", static_cast<int>(monitor_meta.type)));
    }
...
void MonitoringUpdater::start_monitoring() {
    ...
    if (this->device_model.get_optional_value<bool>(ControllerComponentVariables::MonitoringCtrlrEnabled)
        ...
        configPRINTF(("Started monitoring timer with interval: %ld ", process_interval_seconds));
        monitors_timer.interval(std::chrono::seconds(process_interval_seconds));
    } else {
        configPRINTF(("Attempted to start monitoring without 'MonitoringCtrlrEnabled'"));
    }
}
...
void MonitoringUpdater::evaluate_monitor(const VariableMonitoringMeta& monitor_meta, const Component& component,
    ...
    if ((characteristics.dataType == DataEnum::boolean) || (characteristics.dataType == DataEnum::string) ||
        ...
        configPRINTF(("Requested unsupported 'DataEnum' type: %s",
                   conversions::data_enum_to_string(characteristics.dataType)));
        return;
    }

    configPRINTF(("Monitor: %d was triggered on var change with previous value: %d and current: %d",  monitor_meta.monitor,
    ...
    if (monitor_triggered) {
        if (monitor_meta.monitor.type == MonitorEnum::Delta && monitor_trivial) {
            ...
            try {
               configPRINTF(("Updated monitor: %d reference to %d", monitor_meta.monitor, value_current ));

                if (!this->device_model.update_monitor_reference(monitor_id, value_current)) {
                   configPRINTF(("Could not update delta monitor: %d reference!",  monitor_id));
                }
            } catch (const DeviceModelError& e) {
                configPRINTF(("Could not update delta monitor reference with exception "));
            }
        }
        ...
        if (it == std::end(updater_monitors_meta)) {
            ...
            if (!res.second) {
                configPRINTF(("Could not insert monitor to triggered monitor map!"));
                return;
            }
            it = res.first;

            configPRINTF(("Variable: %s with monitor: %d triggered, inserted to updater list", variable.name.get(),
            		monitor_meta.monitor));
        }
        ...
        if (triggered_data.meta_trigger.is_cleared && monitor_meta.monitor.type != MonitorEnum::Delta) {
            ...
            configPRINTF(("Variable: %s triggered already cleared monitor: %s .Setting it back to a 'triggered' state ",
            		variable.name.get(), monitor_meta.monitor ));
        } else if (monitor_meta.monitor.type == MonitorEnum::Delta) {
            // N07.FR.18, N07.FR.19
            ...
            configPRINTF(("Variable: %s triggered delta monitor: %s. Requesting CSMS send", variable.name.get(),  monitor_meta.monitor));
        }
    } else {
        ...
        if (it != std::end(updater_monitors_meta) && monitor_meta.monitor.type != MonitorEnum::Delta) {
            ...
            if (in_triggered_state) {
                ...
                configPRINTF(("Variable: %s marked monitor as cleared: %s", variable.name.get(),
                            monitor_meta.monitor));
            }
        }
    }
...
void MonitoringUpdater::on_variable_changed(const std::unordered_map<int64_t, VariableMonitoringMeta>& monitors,
                                            ...
                                            const VariableAttribute& attribute, const std::string& value_previous,
                                            const std::string& value_current) {
    configPRINTF(("Variable: %s  changed value from: [%s] to: [%s] ", variable.name.get(), value_previous,
                value_current));
    ...
    for (auto& component_variable_monitors : periodic_monitors) {
        for (auto& periodic_monitor_meta : component_variable_monitors.monitors) {
            ...
            if (periodic_monitor_meta.monitor.type == MonitorEnum::Periodic) {
                ...
                configPRINTF(("First aligned timepoint for monitor ID: %d", periodic_monitor_meta.monitor.id));
            } else {
                throw std::runtime_error("Invalid type in periodic monitor list, should never happen!");
            }
            ...
            if (!res.second) {
                configPRINTF(("Could not insert periodic monitor to internal monitor map!"));
                continue;
            }
        }
    }
        ...
        if (monitor.type == MonitorEnum::Periodic) {
            ...
            if (current_time > updater_meta_data.meta_periodic.next_trigger_clock_aligned) {
                ...
                if (distance > static_cast<decltype(distance)>(60)) {
                    configPRINTF(("Missed scheduled monitor time by: %d", distance));
                }
                matches_time = true;
                configPRINTF(("Reporting periodic monitor with id: %d", monitor.id));
            ...
            // Should never happen
            throw std::runtime_error(std::string("Invalid monitor type from: 'get_periodic_monitors': ") +
...
void MonitoringUpdater::process_monitors_internal(bool allow_periodics, bool allow_trigger) {
    ...
    configPRINTF(("Processing internal monitors with periodics: %d and triggers: %s", allow_periodics, allow_trigger));
    ...
    for (auto it = std::begin(updater_monitors_meta); it != std::end(updater_monitors_meta);) {
        ...
        configPRINTF(("Monitor: %d processed %d", updater_monitor_meta.monitor_meta.monitor, should_process));
        ...
        if (!is_offline) {
            if (!updater_monitor_meta.generated_monitor_events.empty()) {
                configPRINTF(("Sent data for monitor: %d", updater_monitor_meta.monitor_meta.monitor));
            ...
            configPRINTF(("We are offline, cached generated events for later!"));
        }
