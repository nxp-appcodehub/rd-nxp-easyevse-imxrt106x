...
#define MIGRATION_FILE_VERSION_V2 2
...
DatabaseHandler::DatabaseHandler(const fs::path& sql_migration_files_path) : DatabaseHandlerCommon(sql_migration_files_path, MIGRATION_FILE_VERSION_V2){
	configPRINTF(("DatabaseHandler is not used."));
}
...
void DatabaseHandler::init_sql() {
    ...
    if (get_stmt->step() == SQLITE_ROW) {
        configPRINTF(("Not clearing tables as there is an ongoing transaction"));
    } else {
...
void DatabaseHandler::inintialize_enum_tables() {
    ...
    if (!this->database->clear_table("METER_VALUE_ITEMS") or !this->database->clear_table("METER_VALUES")) {
        configPRINTF(("Could not clear tables METER_VALUE_ITEMS or METER_VALUES"));
...
void DatabaseHandler::init_enum_table_inner(const std::string& table_name, const int begin, const int end,
    ...
    if (!this->database->clear_table(table_name)) {
        configPRINTF(("Table %s does not exist ", table_name));
        throw QueryExecutionException(this->database->get_error_message());
    }
    ...
    for (int i = begin; i <= end; i++) {
        ...
        if (insert_stmt->step() != SQLITE_DONE) {
            configPRINTF(("Could not perform step."));
            throw QueryExecutionException(this->database->get_error_message());
        }
...
#else
        configPRINTF(("authorization_cache_delete_expired_entries not implemented fully."));
#endif
    } else {
...
#endif
    return conversions::string_to_operational_status_enum("Inoperative");
}
...
void DatabaseHandler::insert_or_update_local_authorization_list_version(int32_t version) {
    ...
    if (stmt->step() != SQLITE_DONE) {
        configPRINTF(("Could not insert or replace into AUTH_LIST_VERSION table"));
        throw QueryExecutionException(this->database->get_error_message());
    }
#endif
}
...
int32_t DatabaseHandler::get_local_authorization_list_version() {
    ...
    if (stmt->step() != SQLITE_ROW) {
        configPRINTF(("Error selecting auth list version"));
        throw QueryExecutionException(this->database->get_error_message());
    }
...
void DatabaseHandler::transaction_metervalues_insert(const std::string& transaction_id, const MeterValue& meter_value) {
    ...
    if (stmt->step() != SQLITE_DONE) {
        configPRINTF(("Could not insert meter values into database"));
        throw QueryExecutionException(this->database->get_error_message());
    }
...
#endif
    return OperationalStatusEnum::Operative; //Inoperative
}
...
std::unique_ptr<EnhancedTransaction> DatabaseHandler::transaction_get(const int32_t evse_id) {
    ...
    if (get_stmt->step() == SQLITE_ROW) {
        ...
        configPRINTF(("There are more than 1 transactions for evse_id %d in the database ", evse_id));
    }
...
DatabaseHandler::get_charging_profiles_matching_criteria(const std::optional<int32_t> evse_id,
    ...
    if (criteria.chargingLimitSource.has_value() && !criteria.chargingLimitSource->empty()) {
        ...
        std::string sources = boost::algorithm::join(
            ...
            ", ");
#else
        std::string sources = "source value";
#endif
...
CiString<20> DatabaseHandler::get_charging_limit_source_for_profile(const int profile_id) {
    ...
    if (stmnt->step() != SQLITE_ROW) {
        configPRINTF(("No record found for %d", profile_id));
    }
    ...
    return res;
#endif
    auto res = CiString<20>("NotImplemented");
    return res;
}
