...
DeviceModelStorageSqlite::DeviceModelStorageSqlite(const fs::path& db_path, const fs::path& migration_files_path,
	...
	if (init_db) {
        if (db_path.empty() || migration_files_path.empty() || config_path.empty()) {
            throw DeviceModelError("Can not initialize device model storage: one of the paths is empty.");
        }
    ...
    if (!db->open_connection()) {
    	throw std::runtime_error("Could not open device model database at provided path: " + db_path.string());
    } else {
        configPRINTF(("Established connection to device model database successfully: %s ", db_path));
    }
#endif
}
...
DeviceModelMap DeviceModelStorageSqlite::get_device_model() {
    ...
    configPRINTF(("Successfully retrieved Device Model from DeviceModelStorage"));
    return device_model;
}
...
bool DeviceModelStorageSqlite::set_variable_attribute_value(const Component& component_id, const Variable& variable_id,
    ...
    if (insert_stmt->step() != SQLITE_DONE) {
        configPRINTF(("error_db_message %s", this->db->get_error_message()));
        return false;
    }
...
ClearMonitoringStatusEnum DeviceModelStorageSqlite::clear_variable_monitor(int monitor_id, bool allow_protected) {
    ...
    if (select_stmt->step() != SQLITE_ROW) {
        configPRINTF(("%s", this->db->get_error_message()));
