...
#else
    this->cs_individual_status = OperationalStatusEnum::Operative; //Inoperative
#endif
...
#else
        OperationalStatusEnum evse_operational = OperationalStatusEnum::Operative; //EVSE
#endif
...
#else
            OperationalStatusEnum connector_operational = OperationalStatusEnum::Operative; //EVSE
#endif
...
void ComponentStateManager::set_cs_individual_operational_status(OperationalStatusEnum new_status, bool persist) {
    ...
    if (persist) {
        try {
            ...
            configPRINTF(("Could not insert charging station availability of id into database: "));
        }
    }
...
void ComponentStateManager::set_evse_individual_operational_status(int32_t evse_id, OperationalStatusEnum new_status,
    ...
    if (persist) {
        try {
            ...
            configPRINTF(("Could not insert evse availability of id %d into database: ", evse_id));
        }
        this->database->insert_evse_availability(evse_id, new_status, true);
    }
...
void ComponentStateManager::set_connector_individual_operational_status(int32_t evse_id, int32_t connector_id,
    ...
    if (persist) {
        try {
            ...
            configPRINTF(("Could not insert connector availability of id %d into database", connector_id));
