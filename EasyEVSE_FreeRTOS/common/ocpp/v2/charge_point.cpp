...
const auto DEFAULT_MESSAGE_QUEUE_SIZE_THRESHOLD = 2E5; //200000
const auto OFFLINE_THRESHOLD_S = 60;


bool dummySendCallback(json message) {
    configPRINTF(("Dummy send callback received message: \r\n"));
    return true;
}
...
ChargePoint::ChargePoint(const std::map<int32_t, int32_t>& evse_connector_structure,
    ...
    if (!this->device_model) {
        throw std::invalid_argument("Device model should not be null");
    }
    ...
    if (!this->callbacks.all_callbacks_valid(this->device_model)) {
        throw std::invalid_argument("All non-optional callbacks must be supplied");
    }

    if (!this->database_handler) {
    	configPRINTF(("Database Handler is null"));
#if DATABASE_ENABLED
        throw std::invalid_argument("Database handler should not be null");
#endif
    }
...
#else
    const std::string firmware_version = "1.0";
#endif
...
void ChargePoint::configure_message_logging_format(const std::string& message_log_path) {
	configPRINTF(("ChargePoint::configure_message_logging_format"));
...
bool ChargePoint::on_charging_state_changed(const uint32_t evse_id, const ChargingStateEnum charging_state,
    ...
    if (transaction == nullptr) {
        configPRINTF(("Can not change charging state: no transaction for evse id %d ", evse_id));
        return false;
    }

    if (transaction->chargingState == charging_state) {
        configPRINTF(("Trying to send charging state changed without actual change, dropping message"));
    } else {
...
//#if DATABASE_ENABLED //ComponentStateManager uses database
...
//#endif
    ...
    if (this->callbacks.cs_effective_operative_status_changed_callback.has_value()) {
    	//component_state_manager is not initialized ;
    ...
    auto transaction_meter_value_callback = [this](const MeterValue& _meter_value, EnhancedTransaction& transaction) {
    	configPRINTF(("TRANSACTION_METER_VALUE_CALLBACK_CALLED____________________"));
        if (_meter_value.sampledValue.empty() or !_meter_value.sampledValue.at(0).context.has_value()) {
            configPRINTF(("Not sending MeterValue due to no values"));
            return;
        }
        ...
        if (type != ReadingContextEnum::Sample_Clock and type != ReadingContextEnum::Sample_Periodic) {
            configPRINTF(("Not sending MeterValue due to wrong context"));
            return;
        }
...
#else
        std::vector<MeasurandEnum> filter_vec = utils::get_measurands_vec(std::string("Energy.Active.Import.Register,Current.Import,Voltage,Power.Active.Import,Power.Reactive.Import,Frequency,SoC"));
        if (type == ReadingContextEnum::Sample_Clock)
        {
        	//AlignedDataMeasurands
        	configPRINTF(("filter_vec with small number of measurands"));
        	filter_vec = utils::get_measurands_vec(std::string("Energy.Active.Import.Register,Voltage"));
        }
//        else
//        {
//        	//SampledDataTxUpdatedMeasurands  (reading context is Sample_Perioridc)
//        	const auto filter_vec = utils::get_measurands_vec(std::string("Energy.Active.Import.Register,Current.Import,Voltage,Power.Active.Import,Power.Reactive.Import,Frequency,SoC"));
//        }
#endif
        ...
        if (!filtered_meter_value.sampledValue.empty()) {
        	configPRINTF(("Sending transaction event request Updated "));
...
#if 0
    configPRINTF(("configuring message logging format\r\n"));
    ...
    if (this->message_queue == nullptr) {
        ...
        try {
            ...
            configPRINTF(("Could not convert configured MessageType value of MessageTypesDiscardForQueueing. Please check you configuration: "));
        } catch (...) {
            configPRINTF(("Could not apply MessageTypesDiscardForQueueing configuration"));
        }
    ...
    this->message_queue = std::make_unique<ocpp::MessageQueue<v2::MessageType>>(
        ...
        MessageQueueConfig<v2::MessageType>{
            3, // attempts
            2, // retry_interval
			3, // DEFAULT_MESSAGE_QUEUE_SIZE_THRESHOLD, 10
			false,
            message_types_discard_for_queueing,
            3} // message_timeout_seconds
            , this->database_handler
		);
}
#endif
...
void ChargePoint::handle_message(const EnhancedMessage<v2::MessageType>& message) {
    ...
    try {
        ...
        configPRINTF(("message type not implemented e.what()" ));
        send_not_implemented_error(message.uniqueId, message.messageTypeId);
    }
}
...
void ChargePoint::message_callback(const std::string& message) {
    ...
    try {
        ...
        configPRINTF(("JSON exception during reception of message: "));
        ...
        configPRINTF(("MalformedRpcMessage exception during handling of message: "));
    ...
    if (enhanced_message.messageTypeId == MessageTypeId::UNKNOWN) {
        configPRINTF(("Cannot handle message with an unknown message type"));
   ...
   //EVSE this->logging->central_system(conversions::messagetype_to_string(enhanced_message.messageType), message);
    try {
        if (this->registration_status == RegistrationStatusEnum::Accepted) {
            ...
            if (enhanced_message.messageType == MessageType::BootNotificationResponse) {
                ...
                if (enhanced_message.messageType == MessageType::GetVariables or
                    ...
                    configPRINTF(("call_error_message %s", call_error_message));
            ...
            if (enhanced_message.messageType == MessageType::BootNotificationResponse) {
                ...
                if (call.msg.requestedMessage == MessageTriggerEnum::BootNotification) {
                    ...
                    configPRINTF(("error_message, %s", error_message));
        ...
        configPRINTF(("Exception during handling of message: "));
        ...
        configPRINTF(("StringConversionException during handling of message: "));
        if (enhanced_message.messageTypeId != MessageTypeId::CALL) {
            return; // CALLERROR shall only follow on a CALL message
        }
        ...
        configPRINTF(("DeviceModelError during handling of message: "));
        auto call_error = CallError(enhanced_message.uniqueId, "GenericError", e.what(), json({}));
        this->message_dispatcher->dispatch_call_error(call_error);
    } catch (json::exception& e) {
        configPRINTF(("JSON exception during handling of message: "));
        if (enhanced_message.messageTypeId != MessageTypeId::CALL) {
            return; // CALLERROR shall only follow on a CALL message
        }
...
void ChargePoint::websocket_connected_callback(const int configuration_slot,
                                               ...
                                               const OcppProtocolVersion ocpp_version) {
	configPRINTF(("websocket_connected_callback()"));
    ...
    if (this->registration_status == RegistrationStatusEnum::Accepted) {
        ...
        if (this->time_disconnected.time_since_epoch() != 0s) {
            ...
            configPRINTF(("getting offline period "));
            // B04.FR.01
...
#endif
            if (offline_duration > std::chrono::seconds(OFFLINE_THRESHOLD_S)) {
                configPRINTF(("offline for more than offline threshold "));
                ...
                configPRINTF(("offline for less than offline threshold "));
...
void ChargePoint::websocket_disconnected_callback(const int configuration_slot,
    ...
    configPRINTF(("websocket_disconnected_callback()"));
...
void ChargePoint::websocket_connection_failed(ConnectionFailedReason reason) {
    ...
    case ConnectionFailedReason::InvalidCSMSCertificate:
        if (!this->skip_invalid_csms_certificate_notifications) {
            ...
            configPRINTF(("Skipping InvalidCsmsCertificate SecurityEvent since it has been sent already"));
        }
        break;
    case ConnectionFailedReason::FailedToAuthenticateAtCsms:
    	configPRINTF(("websocket_connection_failed with reason FailedToAuthenticateAtCsms, sending scurity event request"));
...
void ChargePoint::clear_invalid_charging_profiles() {
    try {
        ...
        configPRINTF(("Found %d evse in the database", evses.size()));
        for (const auto& [evse_id, profiles] : evses) {
            for (auto profile : profiles) {
                try {
                    ...
                    configPRINTF(("Failed database operation for ChargingProfiles: " ));
                }
            }
        }
    } catch (const std::exception& e) {
        configPRINTF(("Unknown error while loading charging profiles from database: " ));
    }
}
...
const std::vector<int32_t>& ChargePoint::get_network_connection_slots() const {
