...
extern ocpp::v2::NetworkConnectionProfile* EVSE_OCPP_GetNetworkConnectionProfile(void);
extern std::string EVSE_OCPP_GetEVSEIdentity(void);
extern void EVSE_OCPP_SetReconnectEvent(void);
extern ocpp::WebsocketConnectionOptions EVSE_OCPP_Get_Connection_Options(void);

namespace {
#if EASYEVSE
const auto WEBSOCKET_INIT_DELAY = std::chrono::seconds(30);
/// \brief Default timeout for the return value (future) of the `configure_network_connection_profile_callback`
///        function.
constexpr int32_t default_network_config_timeout_seconds = 5;
#else
    ...
    for (const auto& network_profile : this->cached_network_connection_profiles) {
        if (network_profile.configurationSlot == configuration_slot) {
        	...
        	if (!this->device_model
                ...
                configPRINTF(("security_profile 0 not officially allowed in OCPP 2.0.1, skipping profile"));
                return std::nullopt;
            }
#endif
...
const std::vector<int32_t>& ConnectivityManager::get_network_connection_slots() const {
...
void ConnectivityManager::connect(std::optional<int32_t> configuration_slot_opt) {
    if (this->network_connection_slots.empty()) {
        configPRINTF(("No network connection profiles configured, aborting websocket connection."));
        return;
    }
    ...
    if (!this->get_network_connection_profile(configuration_slot).has_value()) {
        configPRINTF(("Could not find network connection profile belonging to configuration slot %d",
                      configuration_slot));
        return;
    }
    ...
    if (this->websocket != nullptr && this->is_websocket_connected()) {
    	...
    	configPRINTF(("Trying to connect to websocket..."));
...
#else
    configPRINTF(("Device model not updated"));
#endif
...
#endif
    configPRINTF(("Successful connection confirmed"));
}

void ConnectivityManager::try_connect_websocket() {
	configPRINTF(("Reached try_connect_websocket"));
#if 0 //EVSE
    if (this->device_model.get_value<std::string>(ControllerComponentVariables::ChargePointId).find(':') !=
        std::string::npos) {
        throw std::runtime_error("ChargePointId must not contain \':\'");
    }
#endif
    ...
    configPRINTF(("[m]Configuration slot to set is %d", configuration_slot_to_set));
    if (pending_configuration_slot.has_value())
    {
    	configPRINTF(("[m]Pending configuration slot is %d", pending_configuration_slot.value()));
    }
    else
    {
    	configPRINTF(("[m]Pending configuration slot does not have a value"));
    }
    const std::optional<int> priority_to_set = this->get_priority_from_configuration_slot(configuration_slot_to_set);
    if (priority_to_set.has_value())
    {
    	configPRINTF(("[m]priority_to_set is %d", priority_to_set.value()));
    }
    else
    {
    	configPRINTF(("[m]priority_to_set does not have a value"));
    }
#else
    const int configuration_slot_to_set = 1;
    this->pending_configuration_slot = 1;
    const std::optional<int> priority_to_set = 0;
#endif
    ...
    configPRINTF(("Checking network connection profile..."));

    if (!network_connection_profile.has_value() || !priority_to_set.has_value()) {
        configPRINTF(("No network connection profile configured for %d", configuration_slot_to_set));
        can_use_connection_profile = false;
    } else if (!connection_options.has_value()) {
        configPRINTF(("Connection profile configured for %d failed: not valid URL" , configuration_slot_to_set));
        can_use_connection_profile = false;
        ...
        if (config.has_value() && config->success) {
            ...
            configPRINTF(("Could not use config slot %d", configuration_slot_to_set));
            can_use_connection_profile = false;
        }
    }
    ...
    if (connection_options->security_profile ==
        security::OCPP_1_6_ONLY_UNSECURED_TRANSPORT_WITHOUT_BASIC_AUTHENTICATION) {
        configPRINTF(("Using insecure security profile 0 without authentication"));
    }

    configPRINTF(("Open websocket with NetworkConfigurationPriority: %d",
                  this->active_network_configuration_priority + 1));
#if 0 //EVSE
...
ConnectivityManager::handle_configure_network_connection_profile_callback(int slot,
...
#else
    const int32_t config_timeout = default_network_config_timeout_seconds;
#endif
    ...
    configPRINTF(("Timeout configuring config slot: %d", slot));
    return std::nullopt;
}
...
int ConnectivityManager::get_next_configuration_slot(int32_t configuration_slot) {
    ...
    if (this->network_connection_slots.size() > 1) {
        configPRINTF(("Switching to next network configuration priority"));
    }
...
bool ConnectivityManager::send_to_websocket(const std::string& message) {
    ...
    configPRINTF(("Sending message to websocket"));
...
void ConnectivityManager::on_network_disconnected(OCPPInterfaceEnum ocpp_interface) {
    ...
    if (!network_connection_profile.has_value()) {
        configPRINTF(("Network disconnected. No network connection profile configured"));
...
ConnectivityManager::get_ws_connection_options(const int32_t configuration_slot) {
    ...
    if (!network_connection_profile_opt.has_value()) {
        configPRINTF(("Could not retrieve NetworkProfile of configurationSlot: %d", configuration_slot));
    ...
    WebsocketConnectionOptions connection_options = EVSE_OCPP_Get_Connection_Options();

	auto uri = Uri::parse_and_validate(
			network_connection_profile.ocppCsmsUrl.get(),
			EVSE_OCPP_GetEVSEIdentity(),
			network_connection_profile.securityProfile);

	connection_options.csms_uri = uri;

	connection_options.ocpp_versions.push_back(OcppProtocolVersion::v21);
	connection_options.ocpp_versions.push_back(OcppProtocolVersion::v201);

	connection_options.security_profile = network_connection_profile.securityProfile;
    ...
    return connection_options;
        ...
        configPRINTF(("Could not configure the connection options: %s", e.what()));
    }
...
void ConnectivityManager::on_websocket_stopped_connecting(ocpp::WebsocketCloseReason reason) {
	configPRINTF(("Closed websocket of NetworkConfigurationPriority: %d and close reason: %d",
                  this->active_network_configuration_priority + 1, reason));
	//this->wants_to_be_connected = false; //EVSE -> we don't support multiple active network configurations
    if (this->wants_to_be_connected) {
    	//configPRINTF(("[m]wants_to_be_connected is true"));
        this->websocket_timer.timeout(
            [this, reason] {
                ...
                configPRINTF(("Calling try_connect_websocket"));
#if 1
                EVSE_OCPP_SetReconnectEvent();
#else
                this->try_connect_websocket();
#endif
            },
    ...
    {
    	//configPRINTF(("[m]wants_to_be_connected is false"));
    }
}
...
#if EASYEVSE
    SetNetworkProfileRequest EVSE_profile;
    NetworkConnectionProfile *pEVSE_conn = EVSE_OCPP_GetNetworkConnectionProfile();

    EVSE_profile.configurationSlot = 1;
    EVSE_profile.connectionData = *pEVSE_conn;

    this->cached_network_connection_profiles.push_back(EVSE_profile);
    this->network_connection_slots.push_back(1);
#else
...
void ConnectivityManager::remove_network_connection_profiles_below_actual_security_profile() {
    // Remove all the profiles that are a lower security level than security_level: fix me please
    const auto security_level = this->device_model.get_value<int>(ControllerComponentVariables::SecurityProfile);
