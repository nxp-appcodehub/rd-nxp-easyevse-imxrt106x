...
const auto DEFAULT_BOOT_NOTIFICATION_RETRY_INTERVAL = std::chrono::seconds(10); //30
...
void Provisioning::boot_notification_req(const BootReasonEnum& reason, const bool initiated_by_trigger_message) {
    configPRINTF(("Sending BootNotification"));
...
#else
    charging_station.model = "EVerestModel";
    charging_station.vendorName = "EVerestVendor";
    charging_station.firmwareVersion.emplace("1.0");
    charging_station.serialNumber.emplace("012345678");
#endif
...
void Provisioning::handle_boot_notification_response(CallResult<BootNotificationResponse> call_result) {
    configPRINTF(("Received BootNotificationResponse: { currentTime:%s, interval:%d, status:%s } \nwith messageId %d\r\n",
    		                        call_result.msg.currentTime.to_rfc3339().c_str(), call_result.msg.interval, ocpp::v2::conversions::registration_status_enum_to_string(call_result.msg.status).c_str(), call_result.uniqueId.get().c_str()));
    ...
    if (this->registration_status == RegistrationStatusEnum::Accepted) {
...
#if 0 //EVSE - do not use CSMS as time source
...
#else
        if (this->time_sync_callback.has_value()) {
            this->time_sync_callback.value()(msg.currentTime);
        }
#endif
        ...
        if (msg.interval > 0) {
#if 1
        	configPRINTF(("Setting heartbeat timer interval"));
            this->availability.set_heartbeat_timer_interval(std::chrono::seconds(msg.interval));
#else
            this->availability.set_heartbeat_timer_interval(msg.interval);
#endif
        }
    	...
    	configPRINTF(("Boot notification refused by CSMS"));
    ...
    if (sup_criteria.has_value() and msg.componentCriteria.has_value()) {
        for (const auto& criteria : msg.componentCriteria.value()) {
            ...
            if (sup_criteria.value().find(variable_) == std::string::npos) {
                configPRINTF(("This criteria is not supported: "));
...
void Provisioning::handle_set_network_profile_req(Call<SetNetworkProfileRequest> call) {
    ...
    if (!this->validate_network_profile_callback.has_value()) {
        configPRINTF(("No callback registered to validate network profile"));
    ...
    if (msg.connectionData.securityProfile <
        ...
        configPRINTF(("CSMS attempted to set a network profile with a lower securityProfile"));
        response.status = SetNetworkProfileStatusEnum::Rejected;
        ocpp::CallResult<SetNetworkProfileResponse> call_result(response, call.uniqueId);
        this->context.message_dispatcher.dispatch_call_result(call_result);
        return;
    ...
    if (this->validate_network_profile_callback.value()(msg.configurationSlot, msg.connectionData) !=
        ...
        configPRINTF(("CSMS attempted to set a network profile that could not be validated."));
        response.status = SetNetworkProfileStatusEnum::Rejected;
        ocpp::CallResult<SetNetworkProfileResponse> call_result(response, call.uniqueId);
        this->context.message_dispatcher.dispatch_call_result(call_result);
        return;
    ...
    if (this->context.device_model.set_value(ControllerComponentVariables::NetworkConnectionProfiles.component,
        ...
        configPRINTF(("CSMS attempted to set a network profile that could not be written to the device model"));
        response.status = SetNetworkProfileStatusEnum::Rejected;
        ocpp::CallResult<SetNetworkProfileResponse> call_result(response, call.uniqueId);
        this->context.message_dispatcher.dispatch_call_result(call_result);
        return;
    ...
    configPRINTF(("tech_info: %s", tech_info));
...
void Provisioning::handle_variable_changed(const SetVariableData& set_variable_data) {
...
#else
            this->availability.set_heartbeat_timer_interval(
                            std::stoi(set_variable_data.attributeValue.get()));
#endif
        } catch (const std::invalid_argument& e) {
            configPRINTF(("Invalid argument exception while updating the heartbeat interval: "));
        } catch (const std::out_of_range& e) {
            configPRINTF(("Out of range exception while updating the heartbeat interval: "));
        }
    }
    ...
    if (component_variable_change_requires_websocket_option_update_without_reconnect(component_variable)) {
        configPRINTF(("Reconfigure websocket due to relevant change of ControllerComponentVariable"));
    ...
    for (const auto& [set_variable_data, set_variable_result] : set_variable_results) {
        if (set_variable_result.attributeStatus == SetVariableStatusEnum::Accepted) {
            ...
            if (!mutability.has_value() || (mutability.value() == MutabilityEnum::WriteOnly)) {
                configPRINTF(("Write-only %s : %s changed", set_variable_data.component.name,
                           set_variable_data.variable.name));
            } else {
                configPRINTF(("%s : %s changed to %s", set_variable_data.component.name, set_variable_data.variable.name,
        ...
        try {
            ...
            for (const auto configuration_slot : network_configuration_priorities) {
                ...
                if (network_profile_it == network_connection_profiles.end()) {
                    configPRINTF(("Could not find network profile for configurationSlot: "));
                    return false;
                }
                ...
                if (network_profile.securityProfile == 3 and
                    ...
                    configPRINTF(("SecurityProfile of configurationSlot is 3 but no CSMS Leaf Certificate is installed"));
                    return false;
                }
                if (network_profile.securityProfile >= 2 and
                    ...
                    configPRINTF(("SecurityProfile of configurationSlot is >= 2 but no CSMS Root Certifciate is installed"));
                    return false;
                }
            }
        } catch (const std::invalid_argument& e) {
            configPRINTF(("NetworkConfigurationPriority contains at least one value which is not an integer: "));
            return false;
        } catch (const json::exception& e) {
            configPRINTF(("Could not parse NetworkConnectionProfiles or SetNetworkProfileRequest: "));
            return false;
        }
    }
    return true;
