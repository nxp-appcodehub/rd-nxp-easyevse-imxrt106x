...
void ocpp::v2::MeterValues::update_aligned_data_interval() {
    ...
    configPRINTF(("aligned_meter_values_timer not enabled"));
#if 0 // aligned_meter_values_timer not enabled
...
void ocpp::v2::MeterValues::update_dm_evse_power(const int32_t evse_id, const MeterValue& meter_value) {
    ...
    SetVariableStatusEnum status = this->context.device_model.set_read_only_value(evse_power_cv.component, evse_power_cv.variable.value(),
                                                   AttributeEnum::Actual, std::to_string(power.value()),
                                                   VARIABLE_ATTRIBUTE_VALUE_SOURCE_INTERNAL);
    if (status == SetVariableStatusEnum::Error)
    {
    	configPRINTF(("set_read_only_value failed"));
    }
}
