        ...
        if (response.customData.has_value()) {
            ...
            if (custom_data.contains("updatedPersonalMessageExtra") and is_multilanguage_enabled()) {
                ...
                if (metadata.has_value() and metadata.value().characteristics.valuesList.has_value()) {
                    ...
                    configPRINTF(("DisplayMessageCtrlr variable Language should have a valuesList with supported languages"));
                }
                ...
                for (const auto& m : custom_data.at("updatedPersonalMessageExtra").items()) {
                    ...
                    if (!c.language.has_value()) {
                        configPRINTF(("updated personal message extra sent but language unknown: Can not show message."));
                        continue;
                    }

                    if (supported_languages.empty()) {
                        configPRINTF(("Can not show personal message as the supported languages are unknown (please set the `valuesList` of `DisplayMessageCtrlr` variable `Language` to set the supported languages)"));
                    ...
                    if (std::find(supported_languages.begin(), supported_languages.end(), c.language.value()) !=
                        ...
                        configPRINTF(("Can not send a personal message text in language %s as it is not supported by the charging station.", c.language.value() ));
                    }
                }
            }
        }
...
void TariffAndCost::handle_costupdated_req(const Call<CostUpdatedRequest> call) {
    ...
    if (!transaction_evse_id.has_value()) {
        ...
        configPRINTF(("Received CostUpdatedRequest, but transaction id is not a valid transaction id."));
    }
    ...
    if (!evse_id_opt.has_value()) {
        configPRINTF(("Can not set running cost triggers as there is no evse id found with the transaction id from the incoming CostUpdatedRequest"));
        return;
    }
