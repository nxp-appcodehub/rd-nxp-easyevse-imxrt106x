...
ocpp::v2::Authorization::authorize_req(const IdToken id_token, const std::optional<ocpp::CiString<10000>>& certificate,
    ...
    if (future.wait_for(DEFAULT_WAIT_FOR_FUTURE_TIMEOUT) == std::future_status::timeout) {
        configPRINTF(("Waiting for DataTransfer.conf(Authorize) future timed out!"));
        return response;
    }
...
void ocpp::v2::Authorization::update_authorization_cache_size() {
    ...
    if (auth_cache_size.variable.has_value()) {
        try {
            ...
            configPRINTF(("Could not get authorization cache binary size from database: "));
...
ocpp::v2::Authorization::validate_token(const IdToken id_token, const std::optional<CiString<10000>>& certificate,
    ...
    if (id_token.type == IdTokenEnumStringType::eMAID) {
        ...
        if (is_online and ocsp_request_data.has_value()) {
            configPRINTF(("Online: Pass provided OCSP data to CSMS"));
            ...
            configPRINTF(("Local contract validation result: %d ", local_verify_result));
            ...
            if (is_online) {
                ...
                if (local_verify_result == CertificateValidationResult::IssuerNotFound) {
                    ...
                    if (central_contract_validation_allowed) {
                        configPRINTF(("Online: No local contract root found. Pass contract validation to CSMS"));
                        response = this->authorize_req(id_token, certificate, std::nullopt);
                        forwarded_to_csms = true;
                    } else {
                        configPRINTF(("Online: Central Contract Validation not allowed"));
                    ...
                    if (generated_ocsp_request_data_list.size() > 0) {
                        configPRINTF(("Online: Pass generated OCSP data to CSMS"));
                        response = this->authorize_req(id_token, std::nullopt, generated_ocsp_request_data_list);
                        forwarded_to_csms = true;
                    } else {
                        if (central_contract_validation_allowed) {
                            configPRINTF(("Online: OCSP data could not be generated. Pass contract validation to CSMS"));
                            response = this->authorize_req(id_token, certificate, std::nullopt);
                            forwarded_to_csms = true;
                        } else {
                            configPRINTF(( "Online: OCSP data could not be generated and CentralContractValidation not allowed"));
                            response.idTokenInfo.status = AuthorizationStatusEnum::Invalid;
                        }
                    }
                }
                ...
                if (contract_validation_offline) {
                    configPRINTF(("Offline: contract %d", local_verify_result));
            ...
            configPRINTF(("Can not validate eMAID without certificate chain"));
            response.idTokenInfo.status = AuthorizationStatusEnum::Invalid;
        }
        if (forwarded_to_csms) {
            // AuthorizeRequest sent to CSMS, let's show the results
            configPRINTF(("CSMS idToken status: %d", response.idTokenInfo.status));
            if (response.certificateStatus.has_value()) {
                configPRINTF(("CSMS certificate status: %d", response.certificateStatus.value()));
            }
        }
    ...
    if (this->context.device_model.get_optional_value<bool>(ControllerComponentVariables::LocalAuthListCtrlrEnabled)
        ...
        try {
            ...
            configPRINTF(("Could not request local authorization list entry" ));
        } catch (const std::exception& e) {
            configPRINTF(("Unknown Error while requesting IdTokenInfo: "));
        }
        ...
        if (id_token_info.has_value()) {
            if (id_token_info.value().status == AuthorizationStatusEnum::Accepted) {
                ...
                configPRINTF(("Found valid entry in local authorization list"));
                response.idTokenInfo = id_token_info.value();
            } else if (disabled_remote_auth) {
                configPRINTF(("Found invalid entry in local authorization list but not sending Authorize.req because RemoteAuthorization is disabled"));
                response.idTokenInfo.status = AuthorizationStatusEnum::Unknown;
            } else if (is_online) {
                // C14.FR.03: If a value found but not valid we shall send an authorize request
                configPRINTF(("Found invalid entry in local authorization list: Sending Authorize.req"));
                ...
                configPRINTF(("Found invalid entry in local authorization list whilst offline: Not authorized"));
                response.idTokenInfo.status = AuthorizationStatusEnum::Unknown;
            }
            return response;
        }
    ...
    if (auth_cache_enabled and can_locally_check) {
        try {
            ...
            if (cache_entry.has_value()) {
                ...
                const bool lifetime_expired =
                    lifetime.has_value() and ((cache_entry->last_used.to_time_point() +
                                               lifetime.value()) < now.to_time_point());
                ...
                if (lifetime_expired or cache_expiry_passed) {
                    configPRINTF(("Found valid entry in AuthCache but "));
                    lifetime_expired ? configPRINTF(("lifetime expired")) : configPRINTF(("expiry date passed"));
                    configPRINTF((": Removing from cache and sending new request"));
                    ...
                    configPRINTF(("Found valid entry in AuthCache"));
                    ...
                    return response;
                               ...
                               .value_or(false)) {
                    configPRINTF(("Found invalid entry in AuthCache: Not sending new request because AuthCacheDisablePostAuthorize is enabled"));
                    response.idTokenInfo = id_token_info;
                    return response;
                } else {
                    configPRINTF(("Found invalid entry in AuthCache: Sending new request"));
                }
            }
        } catch (const everest::db::Exception& e) {
            configPRINTF(("Database Error: "));
        } catch (const json::exception& e) {
            configPRINTF(("Could not parse data of IdTokenInfo: "));
        } catch (const std::exception& e) {
            configPRINTF(("Unknown Error while parsing IdTokenInfo: "));
        }
    }
    ...
    if (!is_online and
        this->context.device_model.get_optional_value<bool>(ControllerComponentVariables::OfflineTxForUnknownIdEnabled)
            .value_or(false)) {
        configPRINTF(("Offline authorization due to OfflineTxForUnknownIdEnabled being enabled"));
        response.idTokenInfo.status = AuthorizationStatusEnum::Accepted;
        return response;
    }
    ...
    if (!disabled_remote_auth) {
        ...
        if (auth_cache_enabled) {
            try {
                ...
                configPRINTF(("Could not insert into authorization cache entry: "));
            }
    ...
    configPRINTF(("Not sending Authorize.req because RemoteAuthorization is disabled"));
    ...
    while (true) {
    	{
    		...
    		if (this->auth_cache_cleanup_cv.wait_for(lk, std::chrono::minutes(15), [&]() {
    			...
    			configPRINTF(("Time based authorization cache cleanup"));
    		}
#else
    			if (this->auth_cache_cleanup_cv.wait_for(this->auth_cache_cleanup_mutex, 15*60)) {
    				return; //return !this->auth_cache_cleanup_handler_running or this->auth_cache_cleanup_required;
    			}
#endif
                else {
    				configPRINTF(("Time based authorization cache cleanup"));
        ...
        try {
            ...
            configPRINTF(("Could not delete expired authorization cache entries from database"));
        } catch (const std::exception& e) {
            configPRINTF(("Could not delete expired authorization cache entries from database"));
        }
        ...
        try {
            ...
            if (local_entries.variable.has_value()) {
                try {
                    ...
                    configPRINTF(("Could not set local list count to device model:"));
                } catch (const everest::db::Exception& e) {
                    configPRINTF(("Could not get local list count from database: "));
                } catch (const std::exception& e) {
                    configPRINTF(("Could not get local list count from database: "));
                }
            }
        } catch (const everest::db::Exception& e) {
            configPRINTF(("Could not update local authorization list in database: "));
            response.status = SendLocalListStatusEnum::Failed;
        }
    }
    ...
    if (request.versionNumber <= 0) {
        ...
        if (!request.localAuthorizationList.has_value() or request.localAuthorizationList.value().empty()) {
            try {
                ...
                configPRINTF(("Clearing of local authorization list failed: "));
            }
        } else {
            ...
            if (!has_duplicate_in_list(list) and
                ...
                try {
                    ...
                    status = SendLocalListStatusEnum::Failed;
                    configPRINTF(("Full update of local authorization list failed (at least partially): "));
                }
            }
        }
        ...
        if (request.versionNumber <= this->context.database_handler.get_local_authorization_list_version()) {
            ...
            try {
                ...
                status = SendLocalListStatusEnum::Failed;
                configPRINTF(("Differential update of authorization list failed (at least partially): "));
            }
        }
    }
    return status;
