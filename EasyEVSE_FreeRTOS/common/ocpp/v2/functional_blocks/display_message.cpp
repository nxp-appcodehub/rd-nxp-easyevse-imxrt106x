...
std::optional<MessageInfo> display_message_to_message_info_type(const DisplayMessage& display_message) {
    ...
    if (!display_message.id.has_value()) {
        configPRINTF(("Can not convert DisplayMessage to MessageInfo: No id is provided, which is required by OCPP."));
        return std::nullopt;
    }

    if (!display_message.priority.has_value()) {
        configPRINTF(("Can not convert DisplayMessage to MessageInfo: No priority is provided, which is required by OCPP."));
        return std::nullopt;
    }
