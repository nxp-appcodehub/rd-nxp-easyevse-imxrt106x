...
void Availability::status_notification_req(const int32_t evse_id, const int32_t connector_id,
    ...
    configPRINTF(("Sending status notification request"));
...
void Availability::heartbeat_req(const bool initiated_by_trigger_message) {
    ...
    configPRINTF(("Dispatching heartbeat request"));
...
void Availability::handle_scheduled_change_availability_requests(const int32_t evse_id) {
    if (this->scheduled_change_availability_requests.count(evse_id)) {
        configPRINTF(("Found scheduled ChangeAvailability.req for evse_id:", evse_id));
        ...
        if (!this->context.evse_manager.any_transaction_active(req.evse)) {
            configPRINTF(("Changing availability of evse: %d", evse_id));
            ...
            configPRINTF(("Cannot change availability because transaction is still active"));
        }
    }
}
...
#else
void Availability::set_heartbeat_timer_interval(const uint32_t interval) {
    this->heartbeat_timer.interval = interval;
}
#endif
...
void Availability::handle_change_availability_req(Call<ChangeAvailabilityRequest> call) {
    ...
    if (msg.evse.has_value() and !this->context.evse_manager.is_valid_evse(msg.evse.value())) {
        configPRINTF(("CSMS requested ChangeAvailability for invalid evse id or connector id"));
...
#else
        configPRINTF(("handle_heartbeat_response not implemented fully."));
        unsigned long offsetTime = 0;
        ocpp::DateTime currentTimeCompensated(offsetTime);
#endif
        this->time_sync_callback.value()(currentTimeCompensated);
    }
#else
    configPRINTF(("We don't use Heartbeat as time source."));
#endif
}
