...
void Reservation::handle_reserve_now_request(Call<ReserveNowRequest> call) {
    ...
    if (!reservation_available) {
        ...
        // H01.FR.01
        configPRINTF(("Receiving a reservation request, but reservation is not enabled or implemented."));
    ...
    if (!request.evseId.has_value() &&
        !this->context.device_model
             ...
             .value_or(false)) {
        // H01.FR.19
        configPRINTF(("Trying to make a reservation, but no evse id was given while it should be sent in the request when NonEvseSpecific is disabled."));
    ...
    if (evse_id.has_value()) {
        if (evse_id <= 0 || !this->context.evse_manager.does_evse_exist(evse_id.value())) {
            configPRINTF(("Trying to make a reservation, but evse %d  is not a valid evse id."));
        ...
        if (!this->context.evse_manager.does_connector_exist(
            ...
            configPRINTF(("Trying to make a reservation for connector type for evse but this connector type does not exist."));
        ...
        if (number_of_evses == 0) {
            ...
            configPRINTF(("Trying to make a reservation, but number of evse's is 0"));
            return;
        }
    ...
    if (response.status == ReserveNowStatusEnum::Accepted) {
    	if (reservation_request.evseId.has_value())
    	{
    		configPRINTF(("Reservation with id %d for %d evse_id is accepted", reservation_request.id, reservation_request.evseId.value()));
    	}
    }
}
...
void Reservation::handle_cancel_reservation_callback(Call<CancelReservationRequest> call) {
    ...
    if (this->cancel_reservation_callback == nullptr ||
        ...
        // Reservation not available / implemented, return 'Rejected'.
        // H01.FR.01
        configPRINTF(("Receiving a cancel reservation request, but reservation is not implemented."));
