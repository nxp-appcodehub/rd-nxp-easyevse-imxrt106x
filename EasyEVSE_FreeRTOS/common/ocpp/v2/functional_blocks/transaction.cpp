...
#else
    //const auto meter_value = meter_start;
    /*default SampledDataTxStartedMeasurands is "Energy.Active.Import.Register,Current.Import" */
    const auto meter_value = utils::get_meter_value_with_measurands_applied(
            meter_start, utils::get_measurands_vec(std::string("Energy.Active.Import.Register,Current.Import")));
//    ocpp::DateTime now;
//    const MeterValue meter_value = {.sampledValue = {SampledValue{1.0}, SampledValue{2.0}}, .timestamp = now};
#endif
...
void TransactionBlock::on_transaction_finished(const int32_t evse_id, const DateTime& timestamp,
    ...
    if (enhanced_transaction == nullptr) {
        configPRINTF(("Received notification of finished transaction while no transaction was active"));
        return;
    }
    ...
    try {
        ...
        meter_values = std::make_optional(utils::get_meter_values_with_measurands_applied(
            this->context.database_handler.transaction_metervalues_get_all(enhanced_transaction->transactionId.get()),
            utils::get_measurands_vec(std::string("Energy.Active.Import.Register,Current.Import")),
            utils::get_measurands_vec(std::string("Energy.Active.Import.Register,Current.Import")),
            timestamp,
            this->context.device_model.get_optional_value<bool>(ControllerComponentVariables::SampledDataSignReadings)
                .value_or(false),
        ...
        configPRINTF(("Could not get metervalues of transaction: "));
    }
...
void TransactionBlock::handle_transaction_event_response(const EnhancedMessage<MessageType>& message) {
    ...
    if (!original_msg.idToken.has_value()) {
        configPRINTF(("TransactionEvent.conf contains idTokenInfo when no idToken was part of the TransactionEvent.req"));
        return;
    }
...
#else
    //Whether the Charging Station will deauthorize an ongoing transaction when it receives a non- Accepted
    //authorization status in TransactionEventResponse for this transaction."
    bool StopTxOnInvalidId = true;
    int32_t MaxEnergyOnInvalidId = 0;
    for (auto& evse : this->context.evse_manager) {
            if (auto& transaction = evse.get_transaction();
                transaction != nullptr and transaction->transactionId == original_msg.transactionInfo.transactionId) {
                // Deal with invalid token for transaction
