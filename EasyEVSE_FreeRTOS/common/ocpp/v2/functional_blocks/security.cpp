...
#else
    client_certificate_expiration_check_timer(),
    v2g_certificate_expiration_check_timer()
#endif
{
}
...
Security::on_get_15118_ev_certificate_request(const Get15118EVCertificateRequest& request) {
    ...
    if (!this->context.device_model
             ...
             .value_or(false))
    {
        configPRINTF(("Can not fulfill Get15118EVCertificateRequest because ContractCertificateInstallationEnabled is configured as false!"));
        response.status = Iso15118EVCertificateStatusEnum::Failed;
        return response;
    }

    configPRINTF(("Received Get15118EVCertificateRequest %d", request));
    auto future_res =
    ...
    if (future_res.wait_for(DEFAULT_WAIT_FOR_FUTURE_TIMEOUT) == std::future_status::timeout) {
        configPRINTF(("Waiting for Get15118EVCertificateRequest.conf future timed out!"));
        response.status = Iso15118EVCertificateStatusEnum::Failed;
        return response;
    }

    const auto response_message = future_res.get();
    configPRINTF(("Received Get15118EVCertificateResponse %s", response_message.message));
...
void Security::init_certificate_expiration_check_timers() {
	configPRINTF(("certificate expiration check timers not available in this build"));
    ...
    if (this->context.device_model.get_value<int>(ControllerComponentVariables::SecurityProfile) == 3) {
        this->client_certificate_expiration_check_timer.timeout(
    ...
    this->v2g_certificate_expiration_check_timer.timeout(
        this->context.device_model
...
void Security::security_event_notification_req(const CiString<50>& event_type,
    ...
    configPRINTF(("Sending SecurityEventNotification"));
...
#else
    configPRINTF(("Logging of security event notification disabled"));
#endif
...
void Security::sign_certificate_req(const ocpp::CertificateSigningUseEnum& certificate_signing_use,
    ...
    if (this->awaited_certificate_signing_use_enum.has_value()) {
        configPRINTF(("Not sending new SignCertificate.req because still waiting for CertificateSigned.req from CSMS"));
        return;
    }
    ...
    if (!common.has_value()) {
        configPRINTF(("Missing configuration of commonName to generate CSR"));
        return;
    }

    if (!country.has_value()) {
        configPRINTF(("Missing configuration country to generate CSR"));
        return;
    }

    if (!organization.has_value()) {
        configPRINTF(("Missing configuration of organizationName to generate CSR"));
        return;
    }
    ...
    if (result.status != GetCertificateSignRequestStatus::Accepted or !result.csr.has_value()) {
        configPRINTF(("CSR generation was unsuccessful for sign request: %s",
        		ocpp::conversions::certificate_signing_use_enum_to_string(certificate_signing_use)));

        std::string gen_error = "Sign certificate req failed due to:"; // result.status;
...
void Security::handle_sign_certificate_response(CallResult<SignCertificateResponse> call_result) {
    if (!this->awaited_certificate_signing_use_enum.has_value()) {
        configPRINTF(("Received SignCertificate.conf while not awaiting a CertificateSigned.req . This should not happen."));
        return;
    }
    ...
    if (call_result.msg.status == GenericStatusEnum::Accepted) {
        ...
        if (!cert_signing_wait_minimum.has_value()) {
            configPRINTF(("No CertSigningWaitMinimum is configured, will not attempt to retry SignCertificate.req in case CSMS doesn't send CertificateSigned.req"));
            return;
        }
        if (!cert_signing_repeat_times.has_value()) {
            configPRINTF(("No CertSigningRepeatTimes is configured, will not attempt to retry SignCertificate.req in case CSMS doesn't send CertificateSigned.req"));
            return;
        }
        ...
        int retry_backoff_seconds =
            SEC_MAX(minimum_cert_signing_wait_time_seconds, cert_signing_wait_minimum.value()) *
            std::pow(2, this->csr_attempt); // prevent immediate repetition in case of value 0
        this->certificate_signed_timer.timeout(
            [this]() {
                configPRINTF(("Did not receive CertificateSigned.req in time. Will retry with SignCertificate.req"));
                this->csr_attempt++;
        ...
        this->awaited_certificate_signing_use_enum = std::nullopt;
        this->csr_attempt = 1;
        configPRINTF(("SignCertificate.req has not been accepted by CSMS"));
    }
}

void Security::handle_get_installed_certificate_ids_req(Call<GetInstalledCertificateIdsRequest> call) {
    configPRINTF(("Received GetInstalledCertificateIdsRequest: %s \nwith messageId:", call.msg, call.uniqueId));
...
void Security::handle_install_certificate_req(Call<InstallCertificateRequest> call) {
    configPRINTF(("Received InstallCertificateRequest:%s \nwith messageId: %d", call.msg, call.uniqueId));
...
void Security::handle_delete_certificate_req(Call<DeleteCertificateRequest> call) {
    configPRINTF(("Received DeleteCertificateRequest: %s \nwith messageId: %d", call.msg, call.uniqueId));
...
void Security::scheduled_check_client_certificate_expiration() {
	configPRINTF(("scheduled_check_client_certificate_expiration not available in this build"));
#if 0
    configPRINTF(("Checking if CSMS client certificate has expired"));
    ...
    if (expiry_days_count < 30) {
        configPRINTF(("CSMS client certificate is invalid in %ld days. Requesting new certificate with certificate signing request", expiry_days_count));
        this->sign_certificate_req(ocpp::CertificateSigningUseEnum::ChargingStationCertificate);
    } else {
        configPRINTF(("CSMS client certificate is still valid."));
    }
    ...
    this->client_certificate_expiration_check_timer.interval(std::chrono::seconds(
        this->context.device_model
            ...
            .value_or(12 * 60 * 60)));
#else
    this->client_certificate_expiration_check_timer.interval(std::chrono::seconds(12 * 60 * 60));
#endif
#endif
}

void Security::scheduled_check_v2g_certificate_expiration() {
	configPRINTF(("scheduled_check_v2g_certificate_expiration not available in this build"));
#if 0
    if (this->context.device_model
            .get_optional_value<bool>(ControllerComponentVariables::V2GCertificateInstallationEnabled)
            .value_or(false)) {
        configPRINTF(("Checking if V2GCertificate has expired"));
        int expiry_days_count =
            this->context.evse_security.get_leaf_expiry_days_count(ocpp::CertificateSigningUseEnum::V2GCertificate);
        if (expiry_days_count < 30) {
            configPRINTF(("V2GCertificate is invalid in %ld days. Requesting new certificate with certificate signing request", expiry_days_count));
            this->sign_certificate_req(ocpp::CertificateSigningUseEnum::V2GCertificate);
        } else {
            configPRINTF(("V2GCertificate is still valid."));
        }
    } else {
        if (this->context.device_model.get_optional_value<bool>(ControllerComponentVariables::PnCEnabled)
                .value_or(false)) {
            configPRINTF(("PnC is enabled but V2G certificate installation is not, so no certificate expiration check is performed."));
        }
    }
#if 0
    this->v2g_certificate_expiration_check_timer.interval(std::chrono::seconds(
        this->context.device_model
            ...
            .value_or(12 * 60 * 60)));
#else
    this->v2g_certificate_expiration_check_timer.interval(std::chrono::seconds(12 * 60 * 60));
#endif
#endif
}
