...
#else
    const std::vector<OcppProtocolVersion> supported_versions = {ocpp::OcppProtocolVersion::v201, ocpp::OcppProtocolVersion::v21};
#endif
    if (supported_versions.empty()) {
        throw DeviceModelError("Could not find supported ocpp versions in the InternalCtrlr.");
    }
...
GetVariableStatusEnum DeviceModel::request_value_internal(const Component& component_id, const Variable& variable_id,
    ...
    if (component_it == this->device_model_map.end()) {
        configPRINTF(("unknown component in %s - %s ",component_id.name, variable_id.name));
    ...
    if (variable_it == component.end()) {
        configPRINTF(("unknown variable in %s - %s", component_id.name, variable_id.name));
...
SetVariableStatusEnum DeviceModel::set_value(const Component& component, const Variable& variable,
    ...
    try {
        ...
        configPRINTF(("Could not validate value for component and variable"));
        return SetVariableStatusEnum::Rejected;
    }
...
SetVariableStatusEnum DeviceModel::set_read_only_value(const Component& component, const Variable& variable,
    ...
    configPRINTF(("Not allowed to set read only value for component and variable"));
    return SetVariableStatusEnum::Error;
}
...
void DeviceModel::check_integrity(const std::map<int32_t, int32_t>& evse_connector_structure) {
    configPRINTF(("Checking integrity of device model in storage"));
    try {
        ...
        for (const auto& [component, variable_map] : this->device_model_map) {
        	//configPRINTF(("component name is %s", component.name.get().c_str()));
        ...
        configPRINTF(("Integrity check in Device Model failed: "));
        throw e;
    }
    configPRINTF(("Integrity check in Device Model succeeded"));
}
    ...
    if (found_monitor) {
        try {
            if (this->device_model->update_monitoring_reference(monitor_id, reference_value)) {
                ...
                configPRINTF(("Could not update in DB trivial delta monitor with ID: %d. Reference value not updated!", monitor_id));
            }
        } catch (const everest::db::Exception& e) {
            configPRINTF(("Exception while updating trivial delta monitor reference with ID: %d", monitor_id));
            throw DeviceModelError(e.what());
        }
    } else {
        configPRINTF(("Could not find trivial delta monitor with ID: %d Reference value not updated!", monitor_id));
    }
...
std::vector<SetMonitoringResult> DeviceModel::set_monitors(const std::vector<SetMonitoringData>& requests,
    ...
    for (auto& request : requests) {
        ...
        if (characteristics.supportsMonitoring) {
            configPRINTF(("Validating monitor request of type: and characteristics and value %d", request.value));
            ...
            if (request.type == MonitorEnum::Delta && std::signbit(request.value)) {
                ...
                try {
                    ...
                    configPRINTF(("Could not validate monitor value: %d for component %d and variable %d", request.value,
                                  request.component, request.variable));
                    valid_value = false;
                }
            }
        } else {
        ...
        try {
            ...
            if (monitor_meta.has_value()) {
                ...
                if (request_has_id && monitor_update_listener) {
                    ...
                    if (attribute.has_value()) {
                        ...
                        configPRINTF(("Could not notify monitor update listener, missing variable attribute: "));
                                     // << variable_it->first;
                    }
                }
            ...
            configPRINTF(("Set monitors failed:"));// << e.what();
            throw DeviceModelError(e.what());
        }
...
std::vector<ClearMonitoringResult> DeviceModel::clear_monitors(const std::vector<int32_t>& request_ids,
    ...
    for (auto& id : request_ids) {
        ...
        try {
            ...
            configPRINTF(("Clear monitors failed:"));
            throw DeviceModelError(e.what());
        }
...
int32_t DeviceModel::clear_custom_monitors() {
    try {
        ...
        configPRINTF(("Clear custom monitors failed:"));
        throw DeviceModelError(e.what());
    }
