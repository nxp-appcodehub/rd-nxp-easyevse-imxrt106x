...
#else //bypass lambda issue
    configPRINTF(("Updater thread is not actuallly created"));
    this->updater_thread = std::thread();
    //this->updater_thread = std::thread(&OcspUpdater::updater_thread_loop, this);
#endif
    ...
    while (true) {
...
#else
        this->explicit_update_trigger.wait_until(this->update_ocsp_cache_lock, 200);
#endif
        ...
        try {
            ...
            if (e.allows_retry()) {
                ...
                configPRINTF(("libocpp: OCSP status update failed: will retry."));
                ...
                configPRINTF(("libocpp FATAL: OCSP status update failed: ")); // << e.what();
                throw;
            }
        } catch (UnexpectedMessageTypeFromCSMS& e) {
            configPRINTF(("libocpp: %s, will retry.", e.what()));
            this->update_deadline = std::chrono::steady_clock::now() + this->ocsp_cache_update_retry_interval;
        }
    }
}
...
void OcspUpdater::execute_ocsp_update() {
    ...
    configPRINTF(("libocpp: Updating OCSP cache on %d certificates", ocsp_request_list.size()));
    ...
    configPRINTF(("libocpp: Done updating OCSP cache"));
}
