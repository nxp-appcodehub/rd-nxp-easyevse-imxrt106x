...
void Connector::restore_connector_operative_status() {
    try {
        ...
        configPRINTF(("QueryExecutionException while restoring connector status of evse_id: %d connector_id %d", this->evse_id
                    , this->connector_id )); //<< ": " << e.what();
    } catch (const std::exception& e) {
        configPRINTF(("Internal error while restoring connector status of evse_id: %d, connector_id", this->evse_id
                   , this->connector_id)); // << ": " << e.what();
    }
}
