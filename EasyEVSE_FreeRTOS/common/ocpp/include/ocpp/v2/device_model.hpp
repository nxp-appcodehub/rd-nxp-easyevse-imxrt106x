...
#else
    configPRINTF(("to_specific_type not fully implemented"));
#endif
...
public:
    ...
    T get_value(const RequiredComponentVariable& component_variable,
        ...
        configPRINTF(("Requested for component %s ", component_variable.component.name.get().c_str()));
        if (component_variable.variable.has_value()) {
        	configPRINTF(("Requested for variable %s ", component_variable.variable.value().name.get().c_str()));
        ...
        if (response == GetVariableStatusEnum::Accepted) {
        	...
        	if (response == GetVariableStatusEnum::Rejected)
        		configPRINTF(("Rejected\r\n"));
        	if (response == GetVariableStatusEnum::UnknownVariable)
        	    configPRINTF(("UnknownVariable\r\n"));
        	if (response == GetVariableStatusEnum::UnknownComponent)
        	    configPRINTF(("UnknwonComponent\r\n"));
        	if (response == GetVariableStatusEnum::NotSupportedAttributeType)
        	    configPRINTF(("NotSupportedAttributeType\r\n"));

            configPRINTF(("Directly requested value for ComponentVariable that doesn't exist in the device model"));
    ...
    /// \return List of results of the requested operation
    std::vector<ClearMonitoringResult> clear_monitors(const std::vector<int32_t>& request_ids,
