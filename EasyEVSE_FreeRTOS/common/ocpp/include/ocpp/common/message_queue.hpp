...
#include "freertos_steady_timer.hpp"
...
template <typename M> struct MessageQueueConfig {
    ...
    int message_timeout_seconds = 5;
    int boot_notification_retry_interval_seconds = 5; // interval for BootNotification.req in case response by CSMS is CALLERROR or CSMS does not respond at all
...
private:
    ...
    FreeRTOSSteadyTimer in_flight_timeout_timer;
    FreeRTOSSteadyTimer notify_queue_timer;

    // This timer schedules the resumption of the message queue
    FreeRTOSSteadyTimer resume_timer;
    ...
    void add_to_normal_message_queue(std::shared_ptr<ControlMessage<M>> message) {
        configPRINTF(("Adding message to normal message queue"));
        {
            ...
            if (message->messageType == M::BootNotification) {
                configPRINTF(("Pushing boot notification"));
                this->normal_message_queue.push_front(message);
                configPRINTF(("Pushed boot notification"));
            } else {
            ...
            if (this->config.check_queue(message->messageType)) {
                ...
                try {
                    ...
                    configPRINTF(("Could not insert message into transaction queue"));
                }
            }
#endif
        ...
        this->cv.notify_all();
        configPRINTF(("Notified message queue worker"));
    }
    void add_to_transaction_message_queue(std::shared_ptr<ControlMessage<M>> message) {
    	configPRINTF(("Adding message to transaction message queue"));
        {
            std::lock_guard<std::recursive_mutex> lk(this->message_mutex);
    ...
    void check_queue_sizes() {
        ...
        configPRINTF(("Queue sizes exceed threshold (%d) with %d transaction and %d normal messages in queue",
        		this->config.queues_total_size_threshold,
                this->transaction_message_queue.size(),
                this->normal_message_queue.size()));
    ...
    void drop_messages_from_normal_message_queue() {
        ...
        configPRINTF(("Dropping %d messages from normal message queue.", number_of_dropped_messages));
        ...
        for (int i = 0; i < number_of_dropped_messages; i++) {
            if (this->config.queue_all_messages) {
                ...
                try {
                    ...
                    configPRINTF(("Could not delete message from transaction queue: "));
    ...
    bool drop_update_messages_from_transactional_message_queue() {
        ...
        while (!transaction_message_queue.empty()) {
            ...
            if (remove_next_update_message && element->is_transaction_update_message() &&
                ...
                configPRINTF(("Drop transactional message %s", element->initial_unique_id.get().c_str()));
#if DATABASE_ENABLED
                try {
        ...
        if (drop_count > 0) {
            configPRINTF(("Dropped %d transactional update messages to reduce queue size.", drop_count));
            return true;
        } else {
            configPRINTF(("There are no further transaction update messages to drop!"));
            return false;
        }
    }
    ...
    void resume_now(u_int64_t expected_pause_resume_ctr) {
    	std::lock_guard<std::recursive_mutex> lk(this->message_mutex);
    	configPRINTF(("resume now"));
        if (this->pause_resume_ctr == expected_pause_resume_ctr) {
            this->paused = false;
            this->resuming = false;
            this->cv.notify_one();
            configPRINTF(("resume() notified message queue"));
        }
    }
    ...
    unsigned int current_message_timeout(unsigned int attempt) {
        return (this->config.message_timeout_seconds +
                                    (this->config.transaction_message_retry_interval * attempt));
    }
#endif
...
public:
    ...
    MessageQueue(const std::function<bool(json message)>& send_callback, const MessageQueueConfig<M>& config) {
    	configPRINTF(("In MessageQueue constructor without database handler \r\n"));
    }
    ...
    void start() {
        this->worker_thread = std::thread([this]() {
            ...
            while (this->running) {
                configPRINTF(("Waiting for a message from the message queue"));

                //FreeRTOSRecursiveUniqueLock lk(this->message_mutex);
...
#else
                configPRINTF(("sync method not fully implemented in message queue."));
                this->cv.wait();
#endif
                ...
                configPRINTF(("There are %d messages in the normal message queue. ", this->normal_message_queue.size()));
                configPRINTF(("There are %d messages in the transaction message queue.", this->transaction_message_queue.size()));
                for (int i = 0; i < this->transaction_message_queue.size(); i++)
                {
                    configPRINTF(("transaction message queue: %s", (*this->transaction_message_queue[i]).initial_unique_id.get().c_str()));
                }
                ...
                if (this->in_flight != nullptr) {
                    ...
                    configPRINTF(("There is no message in flight, checking message queue for a new message."));
                }
                ...
                if (message == nullptr) {
                    configPRINTF(("No message in queue ready to be sent yet"));
...

                {
                    ...
                    if (next_message_to_send.has_value()) {
                        if (next_message_to_send.value() != message->uniqueId()) {
                            configPRINTF(("Message with id %d held back because message with id %d should be sent first",
                            		message->uniqueId(), next_message_to_send.value()));
                            continue;
                        }
                    }
                }

                configPRINTF(("Attempting to send message to central system. UID: %s attempt#:  %d ", message->uniqueId().get().c_str(),
                ...
                if (this->message_id_transaction_id_map.count(this->in_flight->message.at(1))) {
                    configPRINTF(("Replacing transaction id"));
                ...
                if (!this->send_callback(this->in_flight->message)) {
                    this->paused = true;
                    configPRINTF(("Could not send message, this is most likely because the charge point is offline."));
                    if (this->in_flight && is_transaction_message(*this->in_flight)) {
                        configPRINTF(("The message in flight is transaction related and will be sent again once the connection can be established again."));
                        ...
                        configPRINTF(("The message in flight  will be sent again once the connection can be established again since QueueAllMessages is set to 'true'."));
                    } else {
                        configPRINTF(("The message in flight is not transaction related and will be dropped"));
                    ...
                    configPRINTF(("Successfully sent message. UID: %s ", this->in_flight->uniqueId().get().c_str()));
                    ...
                    case QueueType::Normal:
                    	configPRINTF(("erasing message from normal queue"));
                        this->normal_message_queue.erase(selected_normal_message_it);
                        break;
                    case QueueType::Transaction:
                    	configPRINTF(("erasing message from transaction queue"));
            ...
            configPRINTF(("Message queue stopped processing messages"));
        });
    }
                ...
                for (auto& persisted_message : persisted_messages) {
                    ...
                    if (ignore_security_event_notifications &&
                        ...
                        try {
                            ...
                            configPRINTF(("Could not delete message from message queue: "));
                        } catch (const std::exception& e) {
                            configPRINTF(("Could not delete message from message queue: "));
                        }
                    } else {
    ...
    void push_call(const json& message, const bool stall_until_accepted = false) {
        ...
        if (is_transaction_message(*control_message)) {
        	...
        	if (this->paused)
        	{
        		configPRINTF(("this->paused is true"));
        	}
        	else
        	{
        		configPRINTF(("this->paused is false"));
        	}

        	if (this->resuming)
        	{
        		configPRINTF(("this->resuming is true"));
        	}
        	else
        	{
        		configPRINTF(("this->resuming is false"));
        	}

        	if (this->config.check_queue(control_message->messageType))
        	{
        		configPRINTF(("this->check_queue is true"));
        	}
        	else
        	{
        		configPRINTF(("this->check_queue is false"));
        	}
#endif
            ...
            if (!this->paused || this->resuming || this->config.check_queue(control_message->messageType) ||
            	...
            	configPRINTF(("Calling add to normal queue from push_call"));
    ...
    std::future<EnhancedMessage<M>> push_call_async(const json& call) {
        ...
        if (!running) {
            ...
            if (this->paused && !this->config.check_queue(message->messageType) && !this->resuming &&
            	...
            	configPRINTF(("Calling add to normal queue from push_call_async"));
    ...
    EnhancedMessage<M> receive(std::string_view message) {
        ...
        if (enhanced_message.messageTypeId == MessageTypeId::CALLRESULT ||
            ...
            //FreeRTOSRecursiveUniqueLock lk(this->message_mutex);
            if (this->in_flight == nullptr) {
                configPRINTF(("Received a CALLRESULT OR CALLERROR without a message in flight, this should not happen"));
                return enhanced_message;
            }
            if (this->in_flight->uniqueId() != enhanced_message.uniqueId) {
                configPRINTF(("Received a CALLRESULT OR CALLERROR with mismatching uid: %s != %s ", this->in_flight->uniqueId().get().c_str(), enhanced_message.uniqueId.get().c_str()));
                configPRINTF(("in flight is: %p", this->in_flight));
                return enhanced_message;
            }
            if (enhanced_message.messageTypeId == MessageTypeId::CALLERROR) {
                configPRINTF(("Received a CALLERROR for message with UID: %s", enhanced_message.uniqueId.get().c_str()));
    ...
    void handle_timeout_or_callerror(const std::optional<EnhancedMessage<M>>& enhanced_message_opt) {
        ...
        if (timeout) {
            configPRINTF(("Message timeout for: (%s) ",
                         this->in_flight->uniqueId().get().c_str()));
        } else {
            configPRINTF(("CALLERROR for: (%s)", this->in_flight->uniqueId().get().c_str()));
        }
        ...
        if (is_transaction_message(*this->in_flight) or this->config.check_queue(this->in_flight->messageType)) {
            if (this->in_flight->message_attempts < this->config.transaction_message_attempts) {
                configPRINTF(("Message shall be persisted and will therefore be sent again"));
                ...
                if (this->config.transaction_message_retry_interval > 0) {
                    ...
                    this->in_flight->timestamp =
                        DateTime(this->in_flight->timestamp.to_time_point() +
                                 this->config.transaction_message_retry_interval *
                                     this->in_flight->message_attempts);
                    configPRINTF(("Retry interval > 0: %d attempting to retry message at: %d ", this->config.transaction_message_retry_interval,
                               this->in_flight->timestamp.to_rfc3339().c_str()));
                } else {
                    // immediate retry
                    this->in_flight->timestamp = DateTime();
                    configPRINTF(("Retry interval of 0 means immediate retry"));
                }

                configPRINTF(("Attempt: %d/%d will be sent at missing_timestamp",
                		this->in_flight->message_attempts + 1,
                        this->config.transaction_message_attempts));

                if (queue_type == QueueType::Transaction) {
                	configPRINTF(("push to the front of the transaction queue"));
                	configPRINTF(("in flight message pushed to transaction queue: %p", this->in_flight));
                    this->transaction_message_queue.push_front(this->in_flight);
                } else if (queue_type == QueueType::Normal) {
                	configPRINTF(("push to the front of the normal queue"));
...
#else
                configPRINTF(("notify_queue_timer.at"));
                this->new_message = true;
                this->cv.notify_all();
#endif
            } else {
                configPRINTF(("Could not deliver message within the configured amount of attempts, dropping message"));
            ...
            configPRINTF(("Message is BootNotification.req and will therefore be sent again"));
            // Generate a new message ID for the retry
            this->in_flight->message[MESSAGE_ID] = ocpp::create_message_id();
            ...
            this->in_flight->timestamp =
                DateTime(this->in_flight->timestamp.to_time_point() +
                         this->config.boot_notification_retry_interval_seconds);
            this->normal_message_queue.push_front(this->in_flight);
#if EASYEVSE
            this->notify_queue_timer.at(
                [this]() {
                ...
                this->in_flight->timestamp.to_time_point());
#else
            configPRINTF(("notify_queue_timer.at(2)"));
            this->new_message = true;
            this->cv.notify_all();
#endif
        } else {
            configPRINTF(("Message is not transaction related, dropping it"));
            if (enhanced_message_opt) {
                this->in_flight->promise.set_value(enhanced_message_opt.value());
            } else {
                EnhancedMessage<M> enhanced_message;
    ...
    void stop() {
        ...
        configPRINTF(("stop() notified message queue"));
    }
    ...
    void resume(std::chrono::seconds delay_on_reconnect) {
        ...
        if (this->pause_resume_ctr > 1 && delay_on_reconnect > std::chrono::seconds(0)) {
            this->resuming = true;
            configPRINTF(("Delaying message queue resume by %ld seconds ", delay_on_reconnect.count()));
    ...
    void add_stopped_transaction_id(std::string stop_transaction_message_id, int32_t transaction_id) {
        configPRINTF(("adding %d for transaction %d ", stop_transaction_message_id, transaction_id));
            ...
            for (auto it = this->transaction_message_queue.begin(); it != transaction_message_queue.end(); ++it) {
                for (const auto& meter_value_message_id :
                    ...
                    if (meter_value_message_id == static_cast<std::string>((*it)->message.at(1))) {
                        configPRINTF(("Adding transactionId %d to MeterValue.req ", transaction_id));
