...
template <typename T> struct ForwardIteratorBase {
    ...
    // New virtual method for safe comparison
    virtual const void* get_raw_iterator() const = 0;
#endif
};
...
#else
    bool not_equal(const base& other) override {
    	return it != *static_cast<const iterator_type*>(other.get_raw_iterator());
    }

    const void* get_raw_iterator() const override {
        return &it;
    }
#endif
