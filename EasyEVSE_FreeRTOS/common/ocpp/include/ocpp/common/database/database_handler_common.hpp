...
protected:
    std::optional<std::unique_ptr<everest::db::sqlite::ConnectionInterface>> database;
...
#if EASYEVSE
    explicit DatabaseHandlerCommon(const fs::path& sql_migration_files_path, uint32_t target_schema_version) noexcept;
#endif
